    ...
    <div className="flex flex-col h-full bg-slate-50 md:rounded-lg overflow-hidden animate-in slide-in-from-bottom duration-300 relative" style={{ fontFamily: '"Times New Roman", Times, serif' }}>
      <div className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar bg-slate-50">
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="space-y-1">
                    <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-wider ml-1">Mã Công Trình</label>
                ...
                <div className="space-y-1">
                    <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-wider ml-1">Tên Công Trình</label>
                    <input value={formData.ten_ct || ''} readOnly className="w-full px-2 py-1.5 bg-slate-50 border border-slate-300 rounded-md text-slate-700 font-bold text-[11px] outline-none h-8"/>
                </div>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div className="space-y-1">
                    <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-wider ml-1">Ngày kiểm tra</label>
                ...
                <div className="space-y-1">
                    <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-wider ml-1">QC Site</label>
        ...
        <div className="space-y-3">
            <h3 className="font-black text-slate-700 uppercase tracking-[0.2em] flex items-center gap-2 border-b border-slate-300 pb-2 px-1 text-xs"><LayoutList className="w-4 h-4 text-amber-700"/> II. TIÊU CHÍ THẨM ĐỊNH LẮP ĐẶT ({formData.items?.length || 0})</h3>
            <div className="space-y-3">
                {formData.items?.map((item, idx) => (
                    <div key={item.id} className="bg-white rounded-xl p-3 border border-slate-200 shadow-sm hover:border-amber-300 transition-colors">
                        ...
                        <textarea 
                            ...
                            className="w-full p-2 bg-slate-50 border border-slate-200 rounded-lg outline-none text-[11px] font-medium focus:ring-1 ring-amber-500 transition-all resize-none h-16" 
                            rows={2}
                            placeholder="Nhận xét hiện trường..."
                        />
                    </div>
                ))}
            </div>
      ...
      <div className="p-4 border-t border-slate-200 bg-white flex justify-end gap-3 shrink-0 shadow-sm z-20 sticky bottom-0">
        ...
        <button onClick={handleSubmit} disabled={isSaving} className="px-10 py-2.5 bg-amber-700 text-white font-black uppercase tracking-[0.2em] rounded-xl shadow-lg shadow-amber-500/20 hover:bg-amber-800 active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50 text-[10px]" type="button">
