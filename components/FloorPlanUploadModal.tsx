...
export const FloorPlanUploadModal: React.FC<FloorPlanUploadModalProps> = ({ projectId, onClose, onSave }) => {
    ...
    return (
        <div className="fixed inset-0 z-[300] bg-slate-900/60 backdrop-blur-md flex items-center justify-center p-4">
            <div className="bg-white dark:bg-slate-900 w-full max-w-xl rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col animate-in zoom-in-95 duration-200">
                <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-white dark:bg-slate-900 shrink-0">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h3 className="font-black text-slate-900 dark:text-slate-100 uppercase text-sm tracking-tight">Upload Floor Plan Layout</h3>
                            <p className="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-1">Hỗ trợ PDF, DWG (Raster), High-res Images</p>
                        </div>
                    </div>
                ...
                <div className="p-8 space-y-6 overflow-y-auto no-scrollbar bg-slate-50 dark:bg-slate-800/50/30">
                    {!file ? (
                        ...
                        <div className="space-y-6">
                            <div className="aspect-[16/10] bg-white dark:bg-slate-900 rounded-3xl border border-slate-200 dark:border-slate-700 overflow-hidden relative shadow-inner group">
                                {previewUrl ? (
                                ...
                                <div className="absolute top-4 right-4 flex gap-2">
                                    <button onClick={() => { setFile(null); setPreviewUrl(null); }} className="bg-white dark:bg-slate-900/90 backdrop-blur-md p-2 rounded-xl text-red-500 dark:text-red-400 shadow-sm hover:bg-red-500 hover:text-white transition-all"><X className="w-4 h-4"/></button>
                                </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div className="md:col-span-2 space-y-1.5">
                                    <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1">Tên Level / Layout Name *</label>
                                    <input 
                                        value={name}
                                        onChange={e => setName(e.target.value)}
                                        className="w-full px-5 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl font-black text-slate-800 dark:text-slate-200 outline-none focus:ring-4 focus:ring-blue-100 uppercase"
                                        placeholder="VD: MẶT BẰNG TẦNG 5 - KHU A"
                                    />
                                </div>
                                <div className="space-y-1.5">
                                    <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1">Version Control</label>
                                    <input 
                                        value={version}
                                        onChange={e => setVersion(e.target.value)}
                                        className="w-full px-5 py-3 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 rounded-2xl font-black text-slate-800 dark:text-slate-200 outline-none focus:ring-4 focus:ring-blue-100"
                                        placeholder="v1.0"
                                    />
                                </div>
                            </div>
                    ...
                    {error && (
                        <div className="bg-red-50 dark:bg-red-900/20 border border-red-100 p-4 rounded-2xl flex items-center gap-3 animate-shake">
                ...
                <div className="p-6 md:p-8 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900 flex flex-col md:flex-row justify-end gap-3 shrink-0">
                    <button onClick={onClose} className="order-2 md:order-1 px-8 py-4 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest hover:text-red-600 dark:text-red-400 transition-colors">Hủy bỏ</button>
                    <button 
