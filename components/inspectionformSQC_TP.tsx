...
export const InspectionFormSQC_TP: React.FC<InspectionFormProps> = ({ initialData, onSave, onCancel, inspections, user, templates }) => {
  const [formData, setFormData] = useState<Partial<Inspection>>({
    id: initialData?.id || `SQC-TP-${Date.now()}`,
    type: 'SQC_TP' as ModuleId,
  ...
  useEffect(() => {
    PersistenceService.hasDraft('SQC_TP', user.id).then(setHasDraft);
  }, []);

  useEffect(() => {
    if (formData.po_number || (formData.materials && formData.materials.length > 0)) {
        PersistenceService.saveDraft('SQC_TP', user.id, formData);
    }
  }, [formData]);

  const recoverDraft = async () => {
    const saved = await PersistenceService.getDraft('SQC_TP', user.id);
    if (saved) {
  ...
  const handleSubmit = async (targetStatus: InspectionStatus = InspectionStatus.PENDING) => {
    ...
    try {
        ...
        (formData.drawingImages || []).forEach((img, idx) => {
            if (img.startsWith('data:')) tasks.push({ url: img, role: 'DRAWING', path: 'DRAWING', originalIndex: idx });
        });
        ...
        const updateStateWithUrl = (task: UploadTask, serverUrl: string) => {
            setFormData(prev => {
                const next = { ...prev };
                if (task.path === 'DELIVERY' && task.originalIndex !== undefined) {
                ...
                } else if (task.path === 'DRAWING' && task.originalIndex !== undefined) {
                    const nextImgs = [...(next.drawingImages || [])];
                    nextImgs[task.originalIndex] = serverUrl;
                    next.drawingImages = nextImgs;
        ...
        const finalForm = formData;
            ...
            await onSave({ ...finalForm,
                ...
                splitRequired: targetStatus !== InspectionStatus.DRAFT && (formData.id || '').split('-').length <= 3, // Split only if it's the base ID
        ...
        alert(`Lỗi lưu báo cáo SQC-TP: ${e.message || "Không thể tải ảnh lên"}`); 
    } finally { 
        setIsSaving(false); 
    }
  };
  ...
  return (
              ...
              <div className="bg-white dark:bg-slate-900/80 backdrop-blur-md border border-blue-200 dark:border-slate-700 p-3 rounded-2xl shadow-xl flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                      ...
                      <div className="flex flex-col">
                          ...
                          <span className="text-[8px] text-slate-500 dark:text-slate-400 dark:text-slate-500 font-bold">Dữ liệu SQC BTP bạn đang nhập chưa được lưu.</span>
                      </div>
                  </div>
              ...
              <div className="bg-white dark:bg-slate-900 p-8 rounded-[2rem] shadow-2xl flex flex-col items-center gap-5 w-[80%] max-w-sm border border-white/20">
                  ...
                  <div className="w-full space-y-2">
                    <p className="text-[11px] font-black text-slate-700 dark:text-slate-300 uppercase tracking-widest text-center">
                        {isLookupLoading ? "Đang truy xuất dữ liệu..." : isSaving ? "Đang tải dữ liệu & ảnh lên server..." : isProcessingImages ? "Đang tải ảnh..." : "Đang xử lý hình ảnh..."}
                    </p>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar pb-28">
        <section className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-4">
            <div className="flex justify-between items-center border-b border-teal-50 pb-2 mb-1">
                ...
                <button onClick={() => setShowHistory(true)} className="px-2 py-1 bg-slate-100 dark:bg-slate-800 hover:bg-teal-100 text-slate-600 dark:text-slate-400 dark:text-slate-500 rounded-lg font-bold uppercase text-[9px] flex items-center gap-1"><History className="w-3 h-3" /> Lịch sử ({historicalRecords.length})</button>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="md:col-span-2">
                    <label className="block text-[9px] font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1 ml-1">Lệnh Sản Xuất / PO Gia Công *</label>
                    <div className="relative flex items-center">
                        <input 
                            value={formData.po_number} 
                            onChange={e => handleInputChange('po_number', e.target.value.toUpperCase())} 
                            onBlur={handlePoBlur}
                            className="w-full px-2 py-1.5 border border-slate-300 dark:border-slate-600 rounded-md font-bold text-[11px] h-9 shadow-inner" 
                            placeholder="Mã LSX..."
                        />
                ...
                <div>
                    <label className="block text-[9px] font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1 ml-1">Đối Tác Gia Công BTP *</label>
                    <input value={formData.supplier || ''} onChange={e => handleInputChange('supplier', e.target.value)} className="w-full px-2 py-1.5 border border-slate-300 dark:border-slate-600 rounded-md font-bold text-[11px] uppercase shadow-inner" placeholder="Tên đơn vị..."/>
                </div>
                <div>
                    <label className="block text-[9px] font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-widest mb-1 ml-1">Địa chỉ xưởng *</label>
                    <div className="flex gap-1.5 items-center">
                        ...
                        <button onClick={handleGetLocation} disabled={isGettingLocation} className="p-2.5 rounded-lg border bg-slate-50 dark:bg-slate-800/50 text-slate-400 dark:text-slate-500"><Locate className="w-4 h-4" /></button>
                        {formData.location && (
                ...
                {/* 1. Supporting Docs */}
                <div className="md:col-span-2 space-y-2 bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-200 dark:border-slate-700">
                    <label className="text-[10px] font-black text-slate-700 dark:text-slate-300 uppercase flex items-center gap-2"><FileText className="w-3.5 h-3.5"/> DANH MỤC TÀI LIỆU HỖ TRỢ</label>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                        {['Bản vẽ có chữ ký', 'Spec', 'Chứng nhận nguyên liệu', 'Rập cỡ có chữ ký', 'Mẫu màu', 'Phiếu thông tin BTP'].map(doc => (
                           <label key={doc} className="flex items-center gap-2 bg-white dark:bg-slate-900 p-2 rounded-lg border border-slate-200 dark:border-slate-700 text-[10px] font-bold text-slate-600 dark:text-slate-400 dark:text-slate-500"><input type="checkbox" className="rounded accent-teal-600"/> {doc}</label>
                        ))}
                    </div>
                </div>
                            ...
                            {formData.deliveryNoteImages?.map((img, i) => (
                                <div key={i} className="relative group shrink-0">
                                    <div className="cursor-zoom-in" onClick={() => setEditorState({ images: formData.deliveryNoteImages!, index: i, context: { type: 'DELIVERY' } })}>
                                        <ProxyImage src={img} alt="Phiếu giao hàng" className="w-10 h-10 rounded border border-slate-200 dark:border-slate-700 object-cover shadow-sm" />
                                    </div>
                                    <button
                            ...
                            {formData.reportImages?.map((img, i) => (
                                <div key={i} className="relative group shrink-0">
                                    <div className="cursor-zoom-in" onClick={() => setEditorState({ images: formData.reportImages!, index: i, context: { type: 'REPORT' } })}>
                                        <ProxyImage src={img} alt="Báo cáo NCC" className="w-10 h-10 rounded border border-slate-200 dark:border-slate-700 object-cover shadow-sm" />
                                    </div>
                                    <button
                ...
                <div className="md:col-span-2 space-y-1 bg-slate-50 dark:bg-slate-800/50 p-2 rounded-lg border border-slate-200 dark:border-slate-700">
                        ...
                        {formData.drawingImages?.map((img, i) => (
                            <div key={i} className="relative group shrink-0">
                                <div className="cursor-zoom-in" onClick={() => setEditorState({ images: formData.drawingImages!, index: i, context: { type: 'DRAWING' } })}>
                                    <ProxyImage src={img} alt="Bản vẽ" className="w-10 h-10 rounded border border-slate-200 dark:border-slate-700 object-cover" />
                                </div>
                                <button
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 px-1">
                ...
                <div className="flex items-center gap-2">
                    ...
                    {(!initialData?.id || initialData.id.split('-').length <= 3) && <button onClick={handleAddMaterial} className="bg-teal-600 text-white p-1.5 rounded-lg shadow active:scale-95 transition-all flex items-center gap-1.5 px-3" type="button">
                        <Plus className="w-3 h-3"/><span className="text-[10px] font-bold uppercase">Thêm Thành Phẩm</span>
                    </button>}
                </div>
            </div>
            ...
            <div className="space-y-3">
                {filteredMaterials.map((mat, matIdx) => {
                    ...
                    <div key={mat.id} className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm overflow-hidden">
                        <div className={`p-3 flex items-center justify-between cursor-pointer transition-colors ${isExp ? 'bg-teal-50/50 border-b border-teal-100' : 'bg-white dark:bg-slate-900'}`} onClick={() => setExpandedMaterial(isExp ? null : mat.id)}>
                            <div className="flex items-center gap-3 flex-1 overflow-hidden"><div className={`w-8 h-8 rounded-lg flex items-center justify-center text-[10px] font-bold shrink-0 ${isExp ? 'bg-teal-600 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 dark:text-slate-500'}`}>{matIdx + 1}</div><div className="flex-1 overflow-hidden"><h4 className="font-bold text-slate-800 dark:text-slate-200 uppercase text-xs truncate">{mat.category || 'BÁN THÀNH PHẨM MỚI'}</h4><div className="flex items-center gap-2 mt-0.5"><span className="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded border border-slate-200 dark:border-slate-700">SL: {mat.deliveryQty} {mat.unit}</span><span className={`text-[9px] font-bold px-1.5 py-0.5 rounded ${hasError ? 'text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-900/20' : 'text-green-600 dark:text-green-500 bg-green-50 dark:bg-green-900/20'}`}>{passRate}% ĐẠT</span></div></div></div>
                            <div className="flex items-center gap-2"><button onClick={(e) => { e.stopPropagation(); if(window.confirm("Xóa?")) setFormData(prev => ({ ...prev, materials: prev.materials?.filter((_, i) => i !== matIdx) })); }} className="p-1.5 text-slate-300 hover:text-red-600 dark:text-red-400" type="button"><Trash2 className="w-4 h-4"/></button>{isExp ? <ChevronUp className="w-4 h-4 text-teal-500"/> : <ChevronDown className="w-4 h-4 text-slate-400 dark:text-slate-500"/>}</div>
                        </div>
                        {isExp && (
                            <div className="p-4 space-y-4 bg-white dark:bg-slate-900 border-t border-slate-50">
                                <div className="grid grid-cols-1 md:grid-cols-12 gap-3 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700">
                                      <div className="col-span-6 md:col-span-3 space-y-1">
                                         <label className="text-[9px] font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase">Mã Định Danh *</label>
                                         <input value={mat.name} onChange={e => updateMaterial(matIdx, 'name', e.target.value)} className="w-full px-2 py-1.5 border border-slate-300 dark:border-slate-600 rounded-md font-bold text-[11px] h-8 shadow-inner uppercase" placeholder="Mã định danh..."/>
                                      </div>
                                      <div className="col-span-6 md:col-span-3 space-y-1">
                                         <label className="text-[9px] font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase">Mã Dự Án</label>
                                         <input value={mat.projectCode || ''} onChange={e => updateMaterial(matIdx, 'projectCode', e.target.value)} className="w-full px-2 py-1.5 border border-slate-300 dark:border-slate-600 rounded-md font-bold text-[11px] h-8 shadow-inner uppercase" placeholder="Mã dự án..."/>
                                      </div>
                                      <div className="col-span-6 md:col-span-3 space-y-1">
                                         <label className="text-[9px] font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase">Tên Dự Án</label>
                                         <input value={mat.projectName || ''} onChange={e => updateMaterial(matIdx, 'projectName', e.target.value)} className="w-full px-2 py-1.5 border border-slate-300 dark:border-slate-600 rounded-md font-bold text-[11px] h-8 shadow-inner" placeholder="Tên dự án..."/>
                                      </div>
                                      <div className="col-span-6 md:col-span-3 space-y-1">
                                      ...
                                      <div className="col-span-4 md:col-span-2 space-y-1">
                                          <label className="text-[9px] font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase">ĐVT</label>
                                          <input value={mat.unit || ''} onChange={e => updateMaterial(matIdx, 'unit', e.target.value)} className="w-full px-2 py-1.5 border border-slate-300 dark:border-slate-600 rounded-md font-bold text-[11px] h-8 shadow-inner uppercase" placeholder="DVT..."/>
                                      </div>
                                      <div className="col-span-4 md:col-span-3 space-y-1">
                                         <label className="text-[9px] font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase">QC Kiểm Tra</label>
                                         <input value={user.name} disabled className="w-full px-2 py-1.5 border border-slate-200 dark:border-slate-700 rounded-md font-bold text-[11px] h-8 bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400 dark:text-slate-500"/>
                                      </div>
                                      <div className="col-span-4 md:col-span-3 space-y-1">
                                          <label className="text-[9px] font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase">Ngày Kiểm Tra</label>
                                          <input type="date" value={mat.date} onChange={e => updateMaterial(matIdx, 'date', e.target.value)} className="w-full px-2 py-1.5 border border-slate-300 dark:border-slate-600 rounded-md font-bold text-[11px] h-8 shadow-inner"/>
                                      </div>
                                 </div>
    ...
    if (['e', 'E', '+', '-'].includes(e.key)) {
...
}} type="text" inputMode="decimal" value={mat.failQty} onChange={e => updateMaterial(matIdx, 'failQty', e.target.value)} className="w-full px-2 py-1 border border-red-300 rounded-md font-bold text-center text-red-700 bg-white dark:bg-slate-900 text-[11px] h-7 shadow-sm"/></div>
                                 </div>
                                 <div className="bg-slate-50 dark:bg-slate-800/50 p-3 rounded-xl border border-slate-200 dark:border-slate-700 space-y-2 mt-2">
                                                    ...
                                                    <div className="flex flex-wrap gap-2 items-center justify-between">
                                                        <div className="flex bg-slate-100 dark:bg-slate-800 p-0.5 rounded-lg gap-0.5 border border-slate-200 dark:border-slate-700 w-fit">
                                                            {[CheckStatus.PASS, CheckStatus.FAIL, CheckStatus.CONDITIONAL].map(st => (
                                                                <button key={st} onClick={() => updateMaterialItem(matIdx, itemIdx, 'status', st)} className={`px-2 py-1.5 rounded-md font-bold uppercase transition-all text-[9px] ${item.status === st ? (st === CheckStatus.PASS ? 'bg-green-600 text-white' : st === CheckStatus.FAIL ? 'bg-red-600 text-white' : 'bg-orange-500 text-white') : 'text-slate-400 dark:text-slate-500 hover:bg-white dark:bg-slate-900'}`} type="button">{st === CheckStatus.PASS ? 'Đạt' : st === CheckStatus.FAIL ? 'Hỏng' : 'ĐK'}</button>
                                                            ))}
                                                        </div>
                                                        ...
                                                        {(item.images || []).map((img, i) => (
                                                            <div key={i} className="relative group shrink-0">
                                                                <div className="cursor-zoom-in" onClick={() => setEditorState({ images: item.images || [], index: i, context: { type: 'ITEM', matIdx, itemIdx } })}>
                                                                    <ProxyImage src={img} alt="Ảnh item" className="w-10 h-10 rounded border border-slate-200 dark:border-slate-700 object-cover shadow-sm" />
                                                                </div>
                                                                <button onClick={() => {
                                                                    const nextMats = [...(formData.materials || [])];
        ...
        <section className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm mt-3">
           <h3 className="text-slate-700 dark:text-slate-300 border-b border-slate-100 dark:border-slate-800 pb-2 mb-3 font-bold uppercase tracking-widest flex items-center gap-2 text-xs"><AlertOctagon className="w-4 h-4 text-amber-500"/> III. GHI CHÚ</h3>
           <textarea
               value={formData.summary || ''}
               onChange={e => handleInputChange('summary', e.target.value)}
               placeholder="Nhập ghi chú tổng hợp (nếu có)..."
               className="w-full border border-slate-200 dark:border-slate-700 rounded-xl p-3 text-sm focus:ring-1 focus:ring-teal-500 outline-none shadow-sm min-h-[80px]"
           />
        </section>
        <section className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm mt-3">
      ...
      <div className="px-4 py-3 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 flex items-center justify-between gap-3 sticky bottom-0 z-40 pb-[calc(env(safe-area-inset-bottom)+0.75rem)]"><button onClick={onCancel} className="h-[44px] px-6 text-slate-500 dark:text-slate-400 dark:text-slate-500 font-bold uppercase tracking-widest rounded-xl border border-slate-200 dark:border-slate-700 text-[10px]" type="button">HỦY BỎ</button><button onClick={() => handleSubmit(InspectionStatus.DRAFT)} disabled={isSaving} className="h-[44px] px-6 text-blue-600 border border-blue-600 bg-blue-50 dark:bg-slate-800 font-bold uppercase tracking-widest rounded-xl shadow-sm text-[10px] whitespace-nowrap" type="button">LƯU NHÁP</button><button onClick={() => handleSubmit(InspectionStatus.PENDING)} disabled={isSaving} className="h-[44px] flex-1 bg-teal-700 text-white font-bold uppercase tracking-widest rounded-xl shadow-lg hover:bg-teal-800 flex items-center justify-center gap-2 disabled:opacity-50 text-[10px]" type="button">{isSaving ? <Loader2 className="w-4 h-4 animate-spin"/> : <Save className="w-4 h-4"/>}<span>GỬI DUYỆT SQC-TP</span></button></div>
