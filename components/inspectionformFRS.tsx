...
export const InspectionFormFRS: React.FC<InspectionFormProps> = ({ initialData, onSave, onCancel, workshops, user }) => {
  ...
  useEffect(() => {
    PersistenceService.hasDraft('FRS', user.id).then(setHasDraft);
  }, []);

  useEffect(() => {
    if (formData.ma_ct || (formData.items && formData.items.length > 0)) {
      PersistenceService.saveDraft('FRS', user.id, formData);
    }
  }, [formData]);

  const recoverDraft = async () => {
    const saved = await PersistenceService.getDraft('FRS', user.id);
    if (saved) {
  ...
  const clearDraft = () => {
    PersistenceService.clearDraft('FRS', user.id);
    setHasDraft(false);
  };
    ...
    try { 
        ...
        if (totalTasks > 0) {
            await Promise.all(tasks.map(async (task) => {
                const serverUrl = await uploadQMSImage(task.url, { entityId, type: 'FRS', role: task.role });
        ...
        console.error("ISO-SAVE-FRS:", e);
        alert("Lỗi khi lưu phiếu FRS: " + (e.message || "Không thể tải ảnh lên")); 
    } finally { 
        setIsSaving(false); 
    }
  };
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-800/50 md:rounded-lg overflow-hidden animate-in slide-in-from-bottom duration-300 relative" style={{ fontFamily: '"Times New Roman", Times, serif' }}>
              ...
              <div className="bg-white dark:bg-slate-900/80 backdrop-blur-md border border-blue-200 dark:border-slate-700 p-3 rounded-2xl shadow-xl flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                      ...
                      <div className="flex flex-col">
                          ...
                          <span className="text-[8px] text-slate-500 dark:text-slate-400 dark:text-slate-500 font-bold">Dữ liệu FRS bạn đang nhập chưa được lưu.</span>
                      </div>
                  </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar bg-slate-50 dark:bg-slate-800/50">
        <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="space-y-1">
                    <label className="block text-[9px] font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-wider ml-1">Mã Dự Án / Mẫu</label>
                    <input value={formData.ma_ct || ''} readOnly className="w-full px-2 py-1.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-300 dark:border-slate-600 rounded-md text-slate-700 dark:text-slate-300 font-bold text-[11px] outline-none h-8"/>
                </div>
                <div className="space-y-1">
                    <label className="block text-[9px] font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-wider ml-1">Tên Sản Phẩm Mẫu</label>
                    <input value={formData.ten_hang_muc || ''} readOnly className="w-full px-2 py-1.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-300 dark:border-slate-600 rounded-md text-slate-700 dark:text-slate-300 font-bold text-[11px] outline-none h-8"/>
                </div>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div className="space-y-1">
                    <label className="block text-[9px] font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-wider ml-1">Ngày kiểm mẫu</label>
                    <input type="date" value={formData.date} onChange={e => handleInputChange('date', e.target.value)} className="w-full px-2 py-1.5 border border-slate-300 dark:border-slate-600 rounded-md font-bold outline-none text-[11px] h-8"/>
                </div>
                <div className="space-y-1">
                    <label className="block text-[9px] font-bold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-wider ml-1">QC/QA Lead</label>
                    <input value={formData.inspectorName || user.name} readOnly className="w-full px-2 py-1.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-300 dark:border-slate-600 rounded-md font-bold text-[11px] h-8"/>
                </div>
            </div>
        ...
        <div className="space-y-3">
            <h3 className="font-black text-slate-700 dark:text-slate-300 uppercase tracking-[0.2em] flex items-center gap-2 border-b border-slate-300 dark:border-slate-600 pb-2 px-1 text-xs"><LayoutList className="w-4 h-4 text-orange-600"/> II. TIÊU CHÍ THẨM ĐỊNH MẪU ĐẦU TIÊN ({formData.items?.length || 0})</h3>
            <div className="space-y-3">
                {formData.items?.map((item, idx) => (
                    <div key={item.id} className="bg-white dark:bg-slate-900 rounded-xl p-3 border border-slate-200 dark:border-slate-700 shadow-sm hover:border-orange-300 transition-colors">
                        <div className="flex justify-between items-start mb-2 border-b border-slate-100 dark:border-slate-800 pb-2">
                            <div className="flex-1">
                                <span className="bg-slate-100 dark:bg-slate-800 text-[8px] font-black uppercase text-slate-500 dark:text-slate-400 dark:text-slate-500 px-2 py-0.5 rounded-full border border-slate-200 dark:border-slate-700">{item.category}</span>
                                <p className="w-full font-black text-xs text-slate-800 dark:text-slate-200 uppercase tracking-tight mt-1">{item.label}</p>
                            </div>
                        </div>
                        <div className="flex bg-slate-50 dark:bg-slate-800/50 p-1 rounded-lg gap-1 border border-slate-200 dark:border-slate-700 shadow-inner w-fit mb-2">
                            {[CheckStatus.PASS, CheckStatus.FAIL].map(st => (
                                <button 
                                    key={st} 
                                    onClick={() => handleItemChange(idx, 'status', st)} 
                                    className={`px-3 py-1 rounded-md text-[9px] font-black uppercase tracking-wider transition-all active:scale-95 ${item.status === st ? (st === CheckStatus.PASS ? 'bg-green-600 text-white shadow-sm' : 'bg-red-600 text-white shadow-sm') : 'text-slate-400 dark:text-slate-500 hover:bg-white dark:bg-slate-900'}`} 
                                    type="button"
                                >
                                    {st}
                                </button>
                        ...
                        <div className="flex items-center gap-2 mb-2">
                            <textarea 
                                ...
                                className="flex-1 p-2 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg outline-none text-[11px] font-medium focus:ring-1 ring-orange-500 transition-all resize-none h-16" 
                                rows={2}
                                placeholder="Nhận xét mẫu..."
                            />
                            <div className="flex flex-col gap-2">
                                <button onClick={() => { setActiveUploadContext({ type: 'ITEM', itemIdx: idx }); cameraInputRef.current?.click(); }} className="p-2 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-400 dark:text-slate-500 hover:text-orange-600" type="button"><Camera className="w-4 h-4" /></button>
                                <button onClick={() => { setActiveUploadContext({ type: 'ITEM', itemIdx: idx }); fileInputRef.current?.click(); }} className="p-2 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-lg text-slate-400 dark:text-slate-500 hover:text-orange-600" type="button"><ImageIcon className="w-4 h-4" /></button>
                            </div>
                        </div>
      ...
      <div className="p-4 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 flex justify-end gap-3 shrink-0 shadow-sm z-20 sticky bottom-0">
        <button onClick={onCancel} className="px-6 py-2 text-slate-500 dark:text-slate-400 dark:text-slate-500 font-bold uppercase tracking-widest hover:bg-slate-50 dark:hover:bg-slate-800/50 dark:bg-slate-800/50 rounded-xl transition-all border border-slate-200 dark:border-slate-700 text-[10px]" type="button">Hủy</button>
