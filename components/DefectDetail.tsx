...
export const DefectDetail: React.FC<DefectDetailProps> = ({ defect, user, onBack, onViewInspection }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-800/50 overflow-hidden relative">
      {/* Header Toolbar */}
      <div className="bg-white dark:bg-slate-900 px-4 py-3 border-b border-slate-200 dark:border-slate-700 flex items-center justify-between sticky top-0 z-40 shadow-sm shrink-0">
        <button onClick={onBack} className="flex items-center gap-1.5 text-slate-600 dark:text-slate-400 dark:text-slate-500 font-bold text-sm px-2 py-1.5 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 dark:bg-slate-800 transition-colors active:scale-95">
            <ArrowLeft className="w-5 h-5"/> Quay lại
        </button>
        <div className="flex items-center gap-2">
            <span className={`px-3 py-1.5 rounded-full text-[10px] font-black uppercase tracking-widest border ${defect.status === 'CLOSED' ? 'bg-green-50 dark:bg-green-900/20 text-green-700 border-green-200 dark:border-green-800' : 'bg-red-50 dark:bg-red-900/20 text-red-700 border-red-200'}`}>{defect.status}</span>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-8 no-scrollbar">
        <div className="max-w-4xl mx-auto space-y-6">
            ...
            {/* Title Block */}
            <div className="bg-white dark:bg-slate-900 rounded-[2.5rem] p-6 md:p-10 border border-slate-200 dark:border-slate-700 shadow-sm relative overflow-hidden">
                ...
                <div className="flex flex-col md:flex-row gap-6 items-start">
                    ...
                    <div className="flex-1 overflow-hidden space-y-3">
                        ...
                        <h1 className="text-2xl md:text-3xl font-black text-slate-900 dark:text-slate-100 leading-tight uppercase tracking-tighter">{defect.description}</h1>
                        <div className="flex flex-wrap items-center gap-4 text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest">
                            <div className="flex items-center gap-1.5"><Calendar className="w-4 h-4 text-blue-500 dark:text-blue-400" /> {defect.date}</div>
                        ...
                        {defect.images && defect.images.length > 0 ? (
                            defect.images.map((img, idx) => (
                                <div 
                                    ...
                                    className="aspect-square rounded-2xl overflow-hidden border border-slate-100 dark:border-slate-800 relative group cursor-zoom-in"
                                >
                            ...
                            <div className="col-span-2 py-12 text-center bg-slate-50 dark:bg-slate-800/50 rounded-3xl border border-dashed border-slate-200 dark:border-slate-700">
                                ...
                                <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase">Chưa có hình ảnh</p>
                            </div>
                        )}
                    </div>
                </div>
                    ...
                    <div className="p-6 space-y-6">
                        <div className="space-y-2">
                            <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest flex items-center gap-2">
                                <AlertTriangle className="w-3.5 h-3.5 text-red-500 dark:text-red-400" /> Nguyên nhân gốc rễ
                            </label>
                        ...
                        <div className="space-y-2">
                            <label className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest flex items-center gap-2">
                                <ShieldCheck className="w-3.5 h-3.5 text-green-600 dark:text-green-500" /> Biện pháp khắc phục
                            </label>
                ...
                <div className="p-8">
                    <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Nhân sự phụ trách</p>
                                <p className="text-sm font-black text-slate-800 dark:text-slate-200 uppercase">{defect.responsiblePerson || 'Chưa phân công'}</p>
                            </div>
                        </div>
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Thời hạn (Deadline)</p>
                                <p className="text-sm font-black text-red-600 dark:text-red-400 font-mono">{defect.deadline || 'ASAP'}</p>
                            </div>
                        </div>
                        <div className="h-10 w-px bg-slate-100 dark:bg-slate-800 hidden md:block"></div>
