...
import { ArrowLeft, Box, Edit3, Trash2, ShieldCheck, ScanEye, CheckCircle2, AlertOctagon, X, Loader2, Eraser, Calendar } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 overflow-hidden relative" style={{ fontFamily: '"Times New Roman", Times, serif' }}>
      <div className="bg-white border-b border-slate-200 p-3 sticky top-0 z-30 shadow-sm shrink-0 flex justify-between items-center">
          <div className="flex items-center gap-2"><button onClick={onBack} className="p-2 hover:bg-slate-100 rounded-xl transition-colors border border-slate-200"><ArrowLeft className="w-4 h-4 text-slate-600" /></button><span className="text-sm font-bold text-slate-900 uppercase">FQC REPORT</span></div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50 pb-24">
        ...
        <div className="space-y-3">
            {inspection.items.map((item, i) => (
                <div key={i} className="bg-white p-3 rounded-xl border border-slate-200 flex items-start gap-3">
                    {/* Fixed: Changed 'Pass' string to CheckStatus.PASS enum to fix unintentional comparison error */}
                    <div className={`mt-1 w-2 h-2 rounded-full ${item.status === CheckStatus.PASS ? 'bg-green-500' : 'bg-red-500'}`}></div>
                    <div className="flex-1">
                        <p className="font-bold text-[11px] text-slate-800">{item.label}</p>
                        {item.notes && <p className="text-[10px] italic text-slate-500 mt-0.5">"{item.notes}"</p>}
                    </div>
                    {/* Fixed: Changed 'Pass' string to CheckStatus.PASS enum to fix unintentional comparison error */}
                    <span className={`text-[8px] font-black px-2 py-0.5 rounded border uppercase ${item.status === CheckStatus.PASS ? 'bg-green-50 text-green-600 border-green-200' : 'bg-red-50 text-red-600 border-red-200'}`}>{item.status}</span>
                </div>
            ))}
        </div>
        <div className="bg-white p-4 rounded-xl border border-slate-200 mt-2">
            <h3 className="text-[10px] font-bold uppercase text-slate-500 border-l-4 border-blue-500 pl-2 mb-2">Xác nhận</h3>
      ...
      {!isApproved && isManager && <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-slate-200 flex justify-end z-40"><button onClick={() => setShowModal(true)} className="bg-blue-600 text-white px-6 py-3 rounded-xl font-bold uppercase text-[10px] shadow-lg">DUYỆT FQC</button></div>}
      {showModal && <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4"><div className="bg-white p-6 rounded-2xl w-full max-w-sm space-y-4"><h3 className="font-bold uppercase text-sm">Phê duyệt FQC</h3><SignaturePad label="Chữ ký Manager" value={managerSig} onChange={setManagerSig}/><div className="flex gap-2"><button onClick={() => setShowModal(false)} className="flex-1 py-2 border rounded-lg text-xs font-bold uppercase">Hủy</button><button onClick={handleApprove} disabled={!managerSig || isProcessing} className="flex-1 py-2 bg-blue-600 text-white rounded-lg text-xs font-bold uppercase">{isProcessing ? <Loader2 className="w-4 h-4 animate-spin mx-auto"/> : 'XÁC NHẬN'}</button></div></div></div>}
    </div>
  );
};
