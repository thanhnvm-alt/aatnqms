  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-800/50 overflow-hidden relative" style={{ fontFamily: '"Times New Roman", Times, serif' }}>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50 dark:bg-slate-800/50 pb-24">
        <div className="bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-300 dark:border-slate-600 shadow-sm relative">
            <h1 className="text-lg font-bold text-slate-900 dark:text-slate-100 uppercase mb-2 flex items-center gap-2"><ClipboardList className="w-5 h-5 text-slate-700 dark:text-slate-300"/> {inspection.ten_hang_muc}</h1>
            <p className="text-[11px] font-bold text-slate-600 dark:text-slate-400 dark:text-slate-500">Dự án: {inspection.ma_ct}</p>
            <div className="absolute right-4 top-4 opacity-10 font-black text-4xl text-slate-800 dark:text-slate-200">SAMPLE</div>
        </div>
