...
import { 
  ...
  Loader2, Sparkles, Send, Building2
} from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 md:rounded-lg md:shadow-xl overflow-hidden animate-in slide-in-from-bottom duration-300 relative" style={{ fontFamily: '"Times New Roman", Times, serif', fontSize: '12pt' }}>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 no-scrollbar bg-slate-50">
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1">Mã Dự Án / PO</label><input value={formData.ma_ct || ''} readOnly className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-500 font-bold text-sm shadow-inner"/></div>
                <div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1">Tên Công Trình</label><input value={formData.ten_ct || ''} readOnly className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-500 font-bold text-sm shadow-inner"/></div>
                <div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1">Tên Hạng Mục</label><input value={formData.ten_hang_muc || ''} readOnly className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-500 font-bold text-sm shadow-inner"/></div>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1">Số lượng IPO</label><input type="number" value={formData.so_luong_ipo || 0} readOnly className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl font-bold text-sm shadow-inner"/></div>
                <div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1">ĐVT</label><input value={formData.dvt || 'PCS'} readOnly className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl font-bold text-sm shadow-inner"/></div>
                <div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1">Ngày kiểm</label><input type="date" value={formData.date} onChange={e => handleInputChange('date', e.target.value)} className="w-full p-2.5 border border-slate-200 rounded-xl font-bold text-sm shadow-inner"/></div>
                <div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1">QC/QA</label><input value={formData.inspectorName || user.name} readOnly className="w-full p-2.5 bg-slate-50 border border-slate-200 rounded-xl font-bold text-sm shadow-inner"/></div>
            </div>
        </div>

        <section className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-3">
            <h3 className="text-blue-700 font-black text-xs uppercase tracking-widest flex items-center gap-2 border-b border-blue-50 pb-2"><ImageIcon className="w-4 h-4"/> II. HÌNH ẢNH HIỆN TRƯỜNG</h3>
            <div className="flex gap-3 overflow-x-auto pb-2 no-scrollbar">
                <div className="flex flex-col gap-2 shrink-0">
                    <button onClick={() => { setActiveUploadId('MAIN'); cameraInputRef.current?.click(); }} className="w-24 h-11 bg-blue-50 border border-blue-200 rounded-xl flex items-center justify-center gap-2 text-blue-600 hover:bg-blue-100 transition-colors shadow-sm" type="button">
                        <Camera className="w-4 h-4"/><span className="text-[8px] font-black uppercase">Camera</span>
                    </button>
                    <button onClick={() => { setActiveUploadId('MAIN'); fileInputRef.current?.click(); }} className="w-24 h-11 bg-slate-50 border border-slate-200 rounded-xl flex items-center justify-center gap-2 text-slate-500 hover:bg-slate-100 transition-colors shadow-sm" type="button">
                        <ImageIcon className="w-4 h-4"/><span className="text-[8px] font-black uppercase">Thiết bị</span>
                    </button>
                </div>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-4">
            <h3 className="text-blue-700 border-b border-blue-50 pb-2 mb-2 font-black text-xs uppercase tracking-widest flex items-center gap-2"><MapPin className="w-4 h-4"/> III. ĐỊA ĐIỂM & SỐ LƯỢNG</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                 <div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1">Xưởng sản xuất</label><select value={formData.workshop || ''} onChange={e => handleInputChange('workshop', e.target.value)} className="w-full p-2.5 border border-slate-200 rounded-xl bg-white text-sm font-bold shadow-sm outline-none"><option value="">-- Chọn xưởng --</option>{(workshops || []).map(ws => <option key={ws.code} value={ws.code}>{ws.name}</option>)}</select></div>
                 <div><label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1 mb-1">Công đoạn kiểm tra *</label><select value={formData.inspectionStage || ''} onChange={e => handleInputChange('inspectionStage', e.target.value)} className="w-full p-2.5 border border-slate-200 rounded-xl bg-white text-sm font-bold shadow-sm outline-none"><option value="">-- Chọn giai đoạn --</option>{(availableStages || []).map(s => <option key={s} value={s}>{s}</option>)}</select></div>
            </div>
                                ...
                                <div className="flex flex-wrap gap-3 items-center">
                                    <div className="flex bg-slate-100 p-1.5 rounded-2xl gap-1 border border-slate-200 shadow-inner w-fit">
                                        {[CheckStatus.PASS, CheckStatus.FAIL, CheckStatus.CONDITIONAL].map(st => (
                                            <button 
                                                key={st} 
                                                onClick={() => handleItemChange(originalIndex, 'status', st)} 
                                                className={`px-4 py-2 rounded-xl text-[9pt] font-black uppercase tracking-tight transition-all active:scale-95 ${
                                                    item.status === st 
                                                    ? (st === CheckStatus.PASS ? 'bg-green-600 text-white shadow-lg shadow-green-200' 
                                                      : st === CheckStatus.FAIL ? 'bg-red-600 text-white shadow-lg shadow-red-200' 
                                    ...
                                    <div className="flex items-center gap-2 ml-auto">
                                        <div className="flex gap-1">
                                            <button onClick={() => { setActiveUploadId(item.id); cameraInputRef.current?.click(); }} className="p-2.5 bg-white border border-slate-200 rounded-xl text-slate-400 hover:text-blue-600 shadow-sm transition-all" title="Chụp Camera" type="button"><Camera className="w-4 h-4"/></button>
                                            <button onClick={() => { setActiveUploadId(item.id); fileInputRef.current?.click(); }} className="p-2.5 bg-white border border-slate-200 rounded-xl text-slate-400 hover:text-blue-600 shadow-sm transition-all" title="Tải ảnh thiết bị" type="button"><ImageIcon className="w-4 h-4"/></button>
                                        </div>
                                        {item.status === CheckStatus.FAIL && <button onClick={() => { setActiveNcrItemIndex(originalIndex); setIsNcrModalOpen(true); }} className="px-4 py-2.5 bg-slate-900 text-white rounded-xl text-[8pt] font-black uppercase tracking-widest flex items-center gap-2 shadow-xl active:scale-95 transition-all" type="button"><AlertOctagon className="w-4 h-4"/> NCR</button>}
                                    </div>
                                </div>
                                <textarea value={item.notes || ''} onChange={e => handleItemChange(originalIndex, 'notes', e.target.value)} className="w-full mt-4 p-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm font-medium focus:bg-white outline-none transition-all shadow-inner h-20" placeholder="Ghi chú kỹ thuật..."/>
                            </div>
                        )
                    ))}
        ...
        <section className="bg-white p-6 rounded-[2.5rem] border border-slate-200 shadow-sm mt-6">
            ...
            <div className="grid grid-cols-1 gap-8">
                <SignaturePad label={`Nhân viên QA/QC ký xác nhận (${user.name})`} value={formData.signature} onChange={sig => setFormData({...formData, signature: sig})} />
            </div>
        </section>
      </div>
      <div className="p-4 md:p-6 border-t border-slate-200 bg-white flex flex-col sm:flex-row justify-end gap-3 shrink-0 shadow-[0_-10px_30px_rgba(0,0,0,0.03)] z-20">
        <button onClick={onCancel} className="px-8 py-3.5 text-slate-500 font-black uppercase text-xs tracking-widest hover:bg-slate-50 rounded-xl transition-all" type="button">Hủy bỏ</button>
