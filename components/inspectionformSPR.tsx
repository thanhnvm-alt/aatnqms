...
export const InspectionFormSPR: React.FC<InspectionFormProps> = ({ initialData, onSave, onCancel, workshops, user }) => {
  ...
  useEffect(() => {
    if (formData.ma_ct || (formData.items && formData.items.length > 0)) {
      PersistenceService.saveDraft('SPR', user.id, formData);
    }
  }, [formData]);

  const recoverDraft = async () => {
    const saved = await PersistenceService.getDraft('SPR', user.id);
    if (saved) {
  ...
  const clearDraft = () => {
    PersistenceService.clearDraft('SPR', user.id);
    setHasDraft(false);
  };
    ...
    try { 
        ...
        if (totalTasks > 0) {
            await Promise.all(tasks.map(async (task) => {
                const serverUrl = await uploadQMSImage(task.url, { entityId, type: 'SPR', role: task.role });
        ...
        console.error("ISO-SAVE-SPR:", e);
        alert("Lỗi khi lưu phiếu SPR: " + (e.message || "Không thể tải ảnh lên")); 
    } finally { 
        setIsSaving(false); 
    }
  };
    ...
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-800/50 md:rounded-lg overflow-hidden animate-in slide-in-from-bottom duration-300 relative" style={{ fontFamily: '"Times New Roman", Times, serif' }}>
              ...
              <div className="bg-white dark:bg-slate-900/80 backdrop-blur-md border border-blue-200 dark:border-slate-700 p-3 rounded-2xl shadow-xl flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                      ...
                      <div className="flex flex-col">
                          ...
                          <span className="text-[8px] text-slate-500 dark:text-slate-400 dark:text-slate-500 font-bold">Dữ liệu SPR bạn đang nhập chưa được lưu.</span>
                      </div>
                  </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 no-scrollbar bg-slate-50 dark:bg-slate-800/50">
        <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-4">
            <h3 className="text-slate-700 dark:text-slate-300 border-b border-slate-50 pb-2 mb-2 font-black uppercase tracking-widest flex items-center gap-2"><Box className="w-4 h-4"/> I. THÔNG TIN MẪU CHUẨN</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1 mb-1">Mã Dự Án / Mã Mẫu</label><input value={formData.ma_ct || ''} readOnly className="w-full p-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-500 dark:text-slate-400 dark:text-slate-500 font-bold outline-none"/></div>
                <div><label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1 mb-1">Tên Mẫu</label><input value={formData.ten_hang_muc || ''} readOnly className="w-full p-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-500 dark:text-slate-400 dark:text-slate-500 font-bold outline-none"/></div>
            </div>
        ...
        <div className="space-y-3">
            <h3 className="font-black text-slate-700 dark:text-slate-300 uppercase tracking-[0.2em] flex items-center gap-2 border-b border-slate-300 dark:border-slate-600 pb-2 px-1 text-xs"><LayoutList className="w-4 h-4 text-slate-600 dark:text-slate-400 dark:text-slate-500"/> II. TIÊU CHÍ KIỂM MẪU ({formData.items?.length || 0})</h3>
                ...
                {formData.items?.map((item, idx) => (
                    <div key={item.id} className="bg-white dark:bg-slate-900 rounded-[2rem] p-5 border border-slate-200 dark:border-slate-700 shadow-sm">
                        ...
                        <div className="flex items-center gap-2 mt-4">
                            <textarea value={item.notes || ''} onChange={e => handleItemChange(idx, 'notes', e.target.value)} className="flex-1 p-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl outline-none h-16" placeholder="Ghi chú mẫu..."/>
