...
export const InspectionFormFQC: React.FC<InspectionFormProps> = ({ initialData, onSave, onCancel, workshops, user }) => {
  ...
  const handleSubmit = async () => {
    ...
    try { 
        ...
        console.error("ISO-SAVE-FQC:", e);
        alert("Lỗi khi lưu phiếu FQC: " + (e.message || "Không thể tải ảnh lên")); 
    } finally { 
        setIsSaving(false); 
    }
  };
    ...
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-800/50 md:rounded-lg overflow-hidden animate-in slide-in-from-bottom duration-300 relative" style={{ fontFamily: '"Times New Roman", Times, serif' }}>
              ...
              <div className="bg-white dark:bg-slate-900/80 backdrop-blur-md border border-blue-200 dark:border-slate-700 p-3 rounded-2xl shadow-xl flex items-center justify-between gap-3">
                  <div className="flex items-center gap-2">
                      ...
                      <div className="flex flex-col">
                          ...
                          <span className="text-[8px] text-slate-500 dark:text-slate-400 dark:text-slate-500 font-bold">Dữ liệu FQC bạn đang nhập chưa được lưu.</span>
                      </div>
                  </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 no-scrollbar bg-slate-50 dark:bg-slate-800/50">
        <div className="bg-white dark:bg-slate-900 p-4 rounded-2xl border border-slate-200 dark:border-slate-700 shadow-sm space-y-4">
            <h3 className="text-blue-700 border-b border-blue-50 pb-2 mb-2 font-black uppercase tracking-widest flex items-center gap-2"><Box className="w-4 h-4"/> I. THÔNG TIN SẢN PHẨM HOÀN THIỆN</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div><label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1 mb-1">Mã Dự Án / Mã SP</label><input value={formData.ma_ct || ''} readOnly className="w-full p-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-500 dark:text-slate-400 dark:text-slate-500 font-bold outline-none"/></div>
                <div><label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1 mb-1">Khu vực kiểm / Xưởng</label><select value={formData.ma_nha_may || ''} onChange={e => handleInputChange('ma_nha_may', e.target.value)} className="w-full p-2.5 border border-slate-200 dark:border-slate-700 rounded-xl bg-white dark:bg-slate-900 font-bold shadow-sm outline-none"><option value="">-- Chọn xưởng --</option>{workshops.map(ws => <option key={ws.code} value={ws.code}>{ws.name}</option>)}</select></div>
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                <div><label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1 mb-1">Ngày kiểm FQC</label><input type="date" value={formData.date} onChange={e => handleInputChange('date', e.target.value)} className="w-full p-2.5 border border-slate-200 dark:border-slate-700 rounded-xl font-bold outline-none"/></div>
                <div><label className="block text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest ml-1 mb-1">QA/QC Phụ trách</label><input value={formData.inspectorName || user.name} readOnly className="w-full p-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl font-bold outline-none"/></div>
            </div>
            ...
            <div className="pt-2 border-t border-slate-50">
                <div className="space-y-1.5 bg-slate-50 dark:bg-slate-800/50/50 p-2 rounded-xl border border-slate-100 dark:border-slate-800">
                    ...
                    <div className="flex gap-1.5 overflow-x-auto no-scrollbar min-h-[40px]">{formData.images?.map((img, i) => (<img key={i} src={getProxyImageUrl(img)} className="w-10 h-10 rounded border border-slate-200 dark:border-slate-700 object-cover shrink-0" onClick={() => handleEditImage(formData.images!, i, { type: 'MAIN' })} referrerPolicy="no-referrer" />))}</div>
                </div>
            </div>
        </div>
        <div className="space-y-3">
            <h3 className="font-black text-slate-700 dark:text-slate-300 uppercase tracking-[0.2em] flex items-center gap-2 border-b border-slate-300 dark:border-slate-600 pb-2 px-1"><LayoutList className="w-4 h-4 text-blue-600 dark:text-blue-400"/> II. TIÊU CHÍ KIỂM TRA HOÀN THIỆN ({formData.items?.length || 0})</h3>
            <div className="space-y-4">
                {formData.items?.map((item, idx) => (
                    <div key={item.id} className="bg-white dark:bg-slate-900 rounded-[2rem] p-5 border border-slate-200 dark:border-slate-700 shadow-sm">
                        <div className="flex justify-between items-start mb-4 border-b border-slate-100 dark:border-slate-800 pb-3">
                            <div className="flex-1"><span className="bg-slate-100 dark:bg-slate-800 text-[8pt] font-black uppercase text-slate-500 dark:text-slate-400 dark:text-slate-500 px-2.5 py-0.5 rounded-full border border-slate-200 dark:border-slate-700">{item.category}</span><p className="w-full font-black text-[11pt] text-slate-800 dark:text-slate-200 uppercase tracking-tight mt-1">{item.label}</p></div>
                        </div>
                        <div className="flex bg-slate-100 dark:bg-slate-800 p-1.5 rounded-2xl gap-1 border border-slate-200 dark:border-slate-700 shadow-inner w-fit">
                            {[CheckStatus.PASS, CheckStatus.FAIL].map(st => (
                                <button key={st} onClick={() => handleItemChange(idx, 'status', st)} className={`px-4 py-2 rounded-xl text-[9pt] font-black uppercase tracking-tight transition-all ${item.status === st ? (st === CheckStatus.PASS ? 'bg-green-600 text-white shadow-lg' : 'bg-red-600 text-white shadow-lg') : 'text-slate-400 dark:text-slate-500'}`} type="button">{st}</button>
                            ))}
                        </div>
                        <div className="flex items-center gap-2 mt-4">
                            <textarea value={item.notes || ''} onChange={e => handleItemChange(idx, 'notes', e.target.value)} className="flex-1 p-4 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-2xl outline-none h-16" placeholder="Ghi chú hoàn thiện..."/>
                            <div className="flex flex-col gap-2">
                                <button onClick={() => { setActiveUploadContext({ type: 'ITEM', itemIdx: idx }); cameraInputRef.current?.click(); }} className="p-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-400 dark:text-slate-500 hover:text-blue-600 dark:text-blue-400" type="button"><Camera className="w-5 h-5" /></button>
                                <button onClick={() => { setActiveUploadContext({ type: 'ITEM', itemIdx: idx }); fileInputRef.current?.click(); }} className="p-2.5 bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 rounded-xl text-slate-400 dark:text-slate-500 hover:text-blue-600 dark:text-blue-400" type="button"><ImageIcon className="w-5 h-5" /></button>
                            </div>
                        </div>
      ...
      <div className="p-4 md:p-6 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 flex justify-end gap-3 shrink-0 shadow-sm z-20">
        <button onClick={onCancel} className="px-8 py-3.5 text-slate-500 dark:text-slate-400 dark:text-slate-500 font-black uppercase tracking-widest hover:bg-slate-50 dark:hover:bg-slate-800/50 dark:bg-slate-800/50 rounded-xl transition-all" type="button">Hủy</button>
        <button onClick={handleSubmit} disabled={isSaving || isProcessingImages} className="px-16 py-4 bg-blue-700 text-white font-black uppercase text-xs tracking-[0.2em] rounded-2xl shadow-2xl hover:bg-blue-800 active:scale-95 transition-all flex items-center justify-center gap-3 disabled:opacity-50" type="button">
            {isSaving ? <Loader2 className="w-5 h-5 animate-spin"/> : <Save className="w-5 h-5"/>}
            <span>HOÀN TẤT FQC</span>
        </button>
      </div>
