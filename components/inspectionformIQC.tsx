...
import { Save, X, Box, FileText, QrCode, Loader2, Building2, UserCheck, Calendar, CheckSquare, Square, PenTool, Eraser, Plus, Trash2, Camera, Image as ImageIcon, ClipboardList, Info, ChevronDown, ChevronUp, Calculator, TrendingUp, Search, Layers } from 'lucide-react';
        ...
        <section className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm space-y-5">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4 pt-2">
                ...
                <div className="col-span-2 md:col-span-1 space-y-1">
                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Tài liệu tham chiếu</label>
                    <div className="flex flex-wrap gap-2 pt-1">
                        {DOC_TYPES.map(doc => (
                            <button key={doc} onClick={() => toggleDoc(doc)} className={`px-2 py-1.5 rounded-lg border text-[9px] font-black uppercase transition-all shadow-sm ${formData.referenceDocs?.includes(doc) ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-400 border-slate-200'}`} type="button">{doc}</button>
                        ))}
                    </div>
                </div>
            </div>
                            ...
                            <div className="p-5 space-y-6 bg-white">
                                ...
                                <div className="space-y-4 bg-slate-50/50 p-4 rounded-[1.5rem] border border-slate-100 shadow-inner">
                                    <div className="flex items-center justify-between mb-1">
                                        <div className="flex items-center gap-2">
                                            ...
                                            <h5 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Số lượng & Thời gian kiểm tra</h5>
                                        </div>
                                    ...
                                    <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                                        <div className="flex items-center gap-3 bg-white p-3 rounded-2xl border border-slate-100 shadow-sm">
                                            ...
                                            <div className="flex-1 space-y-1">
                                                ...
                                                <input type="number" value={mat.passQty} onChange={e => updateMaterial(matIdx, 'passQty', Number(e.target.value))} className="w-full p-2.5 bg-green-50/30 border border-green-100 rounded-xl text-center text-sm font-black text-green-700 outline-none"/>
                                            </div>
                                            <div className="flex-1 space-y-1">
                                                ...
                                                <input type="number" value={mat.failQty} onChange={e => updateMaterial(matIdx, 'failQty', Number(e.target.value))} className="w-full p-2.5 bg-red-50/30 border border-red-100 rounded-xl text-center text-sm font-black text-red-700 outline-none"/>
                                            </div>
                                        </div>
                                ...
                                <div className="space-y-3 bg-slate-50/50 p-4 rounded-[1.5rem] border border-slate-100 shadow-inner">
                                    <div className="flex items-center gap-2 mb-2">
                                        ...
                                        <h5 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Nội dung đánh giá chi tiết & Hình ảnh</h5>
                                    </div>
