...
import { 
  ...
  Palette // Added import for Palette
} from 'lucide-react';
...
export const InspectionDetailFRS: React.FC<InspectionDetailProps> = ({ inspection, user, onBack, onEdit, onDelete, onApprove, onPostComment, workshops = [] }) => {
  ...
  const [showManagerModal, setShowManagerModal] = useState(false); // Renamed from showModal to showManagerModal for clarity
  ...
  const handleManagerApprove = async () => {
      ...
      try {
          await onApprove(inspection.id, managerSig, { managerName: user.name }); // Passed managerName
          alert("FSR Approved!");
          setShowManagerModal(false); // Used setShowManagerModal
          onBack();
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 overflow-hidden relative" style={{ fontFamily: '"Times New Roman", Times, serif' }}>
      <div className="bg-white border-b border-slate-200 p-3 sticky top-0 z-30 shadow-sm shrink-0 flex justify-between items-center">
          <div className="flex items-center gap-2"><button onClick={onBack} className="p-2 hover:bg-slate-100 rounded-xl transition-colors active:scale-90 border border-slate-200"><ArrowLeft className="w-4 h-4 text-slate-600" /></button><span className="text-sm font-bold text-slate-900 uppercase">FSR REPORT</span></div>
      ...
      <div className="flex-1 overflow-y-auto p-3 md:p-6 space-y-4 no-scrollbar pb-40 md:pb-32 bg-slate-50">
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 mt-2">
            ...
            <div className="grid grid-cols-2 gap-4">
                <div className="text-center"><p className="text-[9px] font-bold text-slate-500 uppercase mb-1">QC Thực hiện</p><div className="bg-slate-50 h-20 rounded-lg flex items-center justify-center border">{inspection.signature ? <img src={inspection.signature} className="h-full object-contain"/> : 'N/A'}</div><span className="text-[9px] font-bold uppercase mt-1 block">{inspection.inspectorName}</span></div>
                <div className="text-center"><p className="text-[9px] font-bold text-slate-500 uppercase mb-1">Manager Duyệt</p><div className="bg-slate-50 h-20 rounded-lg flex items-center justify-center border">{inspection.managerSignature ? <img src={inspection.managerSignature} className="h-full object-contain"/> : <span className="text-[9px] text-orange-400">Chờ duyệt</span>}</div><span className="text-[9px] font-bold uppercase mt-1 block">Manager</span></div>
            </div>
        </div>
      </div>
