...
export const InspectionDetailSQC_BTP: React.FC<InspectionDetailProps> = ({ 
  ...
  const handleProductionConfirm = async () => {
      ...
      try {
          ...
          alert("Đã xác nhận từ đại diện Đối tác gia công.");
    ...
    <div className="flex flex-col h-full bg-slate-50 overflow-hidden relative" style={{ fontFamily: '"Times New Roman", Times, serif' }}>
      <div className="bg-white border-b border-slate-200 p-3 sticky top-0 z-30 shadow-sm shrink-0 flex justify-between items-center">
          <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                  <h2 className="text-sm font-bold text-slate-900 uppercase tracking-tight">Báo cáo: SQC - BTP</h2>
                  <div className="flex items-center gap-2">
                      <span className={`px-2 py-0.5 rounded text-[9px] font-bold uppercase border ${isApproved ? 'bg-green-600 text-white border-green-600' : 'bg-orange-50 text-orange-600 border-orange-500'}`}>{inspection.status}</span>
      ...
      <div className="flex-1 overflow-y-auto p-3 md:p-6 space-y-4 no-scrollbar pb-40 md:pb-32 bg-slate-50">
        ...
        <div className="bg-white rounded-xl p-3 border border-slate-200 shadow-sm relative overflow-hidden">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mt-3 pt-3 border-t border-slate-100">
                <InfoRow icon={Box} label="Mã LSX / PO" value={inspection.ma_ct} />
                <InfoRow icon={Building2} label="Đối tác gia công" value={inspection.supplier} iconColor="text-blue-500" />
            ...
            <div className="p-3 border-t border-slate-100 bg-slate-50/30 space-y-3">
                ...
                <div className="flex gap-2 items-end">
                    ...
                    <button 
                        ...
                        className="w-12 h-12 bg-blue-600 text-white rounded-xl shadow-md flex items-center justify-center active:scale-95 disabled:opacity-50 transition-all shrink-0"
                        type="button"
                    >
