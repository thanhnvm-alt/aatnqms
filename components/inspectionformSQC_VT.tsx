...
export const InspectionFormSQC_VT: React.FC<InspectionFormProps> = ({ initialData, onSave, onCancel, inspections, user, templates }) => {
  const [formData, setFormData] = useState<Partial<Inspection>>({
    ...
    type: 'SQC_MAT' as ModuleId, // Assuming SQC_VT uses SQC_MAT module type
  ...
  const sqcMatGroups = useMemo(() => {
      const sqcMatTpl = templates['SQC_MAT'] || [];
      return Array.from(new Set(sqcMatTpl.map(i => i.category))).filter(Boolean).sort();
  }, [templates]);
  ...
  const handleSubmit = async () => {
    ...
    } catch (e) { alert("Lỗi lưu SQC-VT."); } finally { setIsSaving(false); }
  };
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 md:rounded-lg overflow-hidden animate-in slide-in-from-bottom duration-300 relative no-scroll-x" style={{ fontFamily: '"Times New Roman", Times, serif' }}>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar pb-28">
        <section className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                ...
                <div>
                    <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1 ml-1">Đối Tác Gia Công Vật Tư *</label>
        ...
        <section className="space-y-3">
            <div className="flex justify-between items-center px-1"><h3 className="font-bold text-slate-700 uppercase tracking-widest flex items-center gap-2 text-xs"><ClipboardList className="w-4 h-4 text-teal-600"/> II. DANH MỤC VẬT TƯ KIỂM TRA ({formData.materials?.length || 0})</h3><button onClick={handleAddMaterial} className="bg-teal-600 text-white p-1.5 rounded-lg shadow active:scale-95 transition-all flex items-center gap-1.5 px-3" type="button"><Plus className="w-3 h-3"/><span className="text-[10px] font-bold uppercase">Thêm Vật Tư</span></button></div>
                ...
                {(formData.materials || []).map((mat, matIdx) => {
                    ...
                    <div key={mat.id} className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                        <div className={`p-3 flex items-center justify-between cursor-pointer transition-colors ${isExp ? 'bg-teal-50/50 border-b border-teal-100' : 'bg-white'}`} onClick={() => setExpandedMaterial(isExp ? null : mat.id)}>
                            <div className="flex items-center gap-3 flex-1 overflow-hidden"><div className={`w-8 h-8 rounded-lg flex items-center justify-center text-[10px] font-bold shrink-0 ${isExp ? 'bg-teal-600 text-white' : 'bg-slate-100 text-slate-500'}`}>{matIdx + 1}</div><div className="flex-1 overflow-hidden"><h4 className="font-bold text-slate-800 uppercase text-xs truncate">{mat.name || 'VẬT TƯ MỚI'}</h4><div className="flex items-center gap-2 mt-0.5"><span className="text-[9px] font-bold text-slate-400 uppercase bg-slate-100 px-1.5 py-0.5 rounded border border-slate-200">SL: {mat.deliveryQty} {mat.unit}</span><span className="text-[9px] font-bold text-green-600 uppercase bg-green-50 px-1.5 py-0.5 rounded">{passRate}% ĐẠT</span></div></div></div>
                                ...
                                <div className="grid grid-cols-12 gap-3">
                                  <div className="col-span-4">
                                      ...
                                      <select value={mat.category || ''} onChange={e => updateMaterial(matIdx, 'category', e.target.value)} className="w-full px-2 py-1.5 border border-slate-300 rounded-md font-bold outline-none text-[10px] h-9 bg-white shadow-sm">
                                          ...
                                          {sqcMatGroups.map(g => <option key={g} value={g}>{g}</option>)}
                                      </select>
                                  </div>
                                  <div className="col-span-3">
                                      <label className="block text-[9px] font-black text-teal-600 uppercase tracking-widest mb-1">Loại kiểm</label>
                                      <select value={mat.inspectType || '100%'} onChange={e => updateMaterial(matIdx, 'inspectType', e.target.value as any)} className="w-full px-2 py-1.5 border border-teal-300 rounded-lg font-black text-[10px] h-9 bg-white outline-none text-teal-700 shadow-sm"><option value="100%">100%</option><option value="AQL">AQL</option></select>
                                  </div>
                                  <div className="col-span-5">
                                      <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-widest mb-1">Tên Vật Tư *</label>
                                      <input value={mat.name} onChange={e => updateMaterial(matIdx, 'name', e.target.value)} className="w-full px-2 py-1.5 border border-slate-300 rounded-lg font-bold focus:ring-1 ring-teal-500 outline-none text-[10px] h-9 shadow-sm" placeholder="Tên sản phẩm..."/>
                                  </div>
                                </div>
                                        ...
                                        <div key={item.id} className={`bg-white border rounded-xl p-3 shadow-sm transition-colors ${item.status === CheckStatus.PASS ? 'border-green-100 hover:border-green-300' : item.status === CheckStatus.FAIL ? 'border-red-100 hover:border-red-300' : item.status === CheckStatus.CONDITIONAL ? 'border-amber-100 hover:border-amber-300' : 'border-slate-200 hover:border-teal-300'}`}>
                                            ...
                                            <div className="flex items-center gap-2 shrink-0 border-t border-slate-50 pt-2">
                                                ...
                                                <div className="flex items-center gap-2 ml-auto">
                                                    <button onClick={() => { setActiveUploadContext({ type: 'ITEM', matIdx, itemIdx }); cameraInputRef.current?.click(); }} className="p-1.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-400 hover:text-teal-600" type="button"><Camera className="w-4 h-4" /></button>
                                                    <button onClick={() => { setActiveUploadContext({ type: 'ITEM', matIdx, itemIdx }); fileInputRef.current?.click(); }} className="p-1.5 bg-slate-50 border border-slate-200 rounded-lg text-slate-400 hover:text-teal-600" type="button"><ImageIcon className="w-4 h-4" /></button>
                                                </div>
                                            </div>
      ...
      <div className="px-4 py-3 border-t border-slate-200 bg-white flex items-center justify-between gap-3 sticky bottom-0 z-40 pb-[calc(env(safe-area-inset-bottom)+0.75rem)]"><button onClick={onCancel} className="h-[44px] px-6 text-slate-500 font-bold uppercase tracking-widest rounded-xl border border-slate-200 text-[10px]" type="button">HỦY BỎ</button><button onClick={handleSubmit} disabled={isSaving || isProcessingImages} className="h-[44px] flex-1 bg-teal-700 text-white font-bold uppercase tracking-widest rounded-xl shadow-lg hover:bg-teal-800 flex items-center justify-center gap-2 disabled:opacity-50 text-[10px]" type="button">{isSaving ? <Loader2 className="w-4 h-4 animate-spin"/> : <Save className="w-4 h-4"/>}<span>GỬI DUYỆT SQC-VT</span></button></div>
