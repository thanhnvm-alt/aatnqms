...
export const InspectionFormSQC_VT: React.FC<InspectionFormProps> = ({ initialData, onSave, onCancel, plans, workshops, user }) => {
  ...
  const handleSubmit = async () => {
    ...
    catch (e) { alert("Lỗi khi lưu phiếu SQC-VT."); } finally { setIsSaving(false); }
  };
    ...
    <div className="flex flex-col h-full bg-slate-50 md:rounded-lg overflow-hidden animate-in slide-in-from-bottom duration-300 relative" style={{ fontFamily: '"Times New Roman", Times, serif' }}>
      <div className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar">
        <div className="bg-white p-4 rounded-xl border border-slate-200 shadow-sm space-y-4">
            <h3 className="text-teal-700 border-b border-teal-50 pb-2 mb-1 font-black uppercase tracking-widest flex items-center gap-2 text-xs"><Box className="w-4 h-4"/> I. THÔNG TIN GIA CÔNG</h3>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div className="space-y-1">
                    <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-wider ml-1">Mã Dự Án / PO</label>
                    <input value={formData.ma_ct || ''} readOnly className="w-full px-2 py-1.5 bg-slate-50 border border-slate-300 rounded-md text-slate-700 font-bold text-[11px] outline-none shadow-inner h-8"/>
                </div>
                <div className="space-y-1">
                    <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-wider ml-1">Nhà thầu phụ / Xưởng gia công</label>
                    <div className="relative">
                        <select value={formData.ma_nha_may || ''} onChange={e => handleInputChange('ma_nha_may', e.target.value)} className="w-full px-2 py-1.5 border border-slate-300 rounded-md bg-white font-bold shadow-sm outline-none text-[11px] appearance-none h-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div className="space-y-1">
                    <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-wider ml-1">Ngày kiểm</label>
                ...
                <div className="space-y-1">
                    <label className="block text-[9px] font-bold text-slate-500 uppercase tracking-wider ml-1">QC Người kiểm</label>
        ...
        <div className="space-y-3">
            <h3 className="font-black text-slate-700 uppercase tracking-[0.2em] flex items-center gap-2 border-b border-slate-300 pb-2 px-1 text-xs"><LayoutList className="w-4 h-4 text-teal-600"/> II. DANH MỤC KIỂM TRA CHẤT LƯỢNG VẬT TƯ</h3>
            <div className="space-y-3">
                {formData.items?.map((item, idx) => (
                    <div key={item.id} className="bg-white rounded-xl p-3 border border-slate-200 shadow-sm hover:border-teal-300 transition-colors">
                        ...
                        <textarea 
                            ...
                            className="w-full p-2 bg-slate-50 border border-slate-200 rounded-lg outline-none text-[11px] font-medium focus:ring-1 ring-teal-500 transition-all resize-none h-16" 
                            rows={2}
      ...
      <div className="p-4 border-t border-slate-200 bg-white flex justify-end gap-3 shrink-0 shadow-sm z-20 sticky bottom-0">
        ...
        <button onClick={handleSubmit} disabled={isSaving} className="px-10 py-2.5 bg-teal-700 text-white font-black uppercase tracking-[0.2em] rounded-xl shadow-lg hover:bg-teal-800 active:scale-95 transition-all flex items-center justify-center gap-2 disabled:opacity-50 text-[10px]" type="button">
            {isSaving ? <Loader2 className="w-4 h-4 animate-spin"/> : <Save className="w-4 h-4"/>}
            <span>HOÀN TẤT SQC-VT</span>
        </button>
      </div>
    </div>
  );
