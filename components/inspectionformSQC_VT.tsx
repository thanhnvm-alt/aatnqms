...
export const InspectionFormSQC_VT: React.FC<InspectionFormProps> = ({ initialData, onSave, onCancel, plans, workshops, user }) => {
  const [formData, setFormData] = useState<Partial<Inspection>>({ id: `SQC-VT-${Date.now()}`, date: new Date().toISOString().split('T')[0], status: InspectionStatus.DRAFT, items: [], images: [], score: 0, ...initialData });
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 md:rounded-lg overflow-hidden animate-in slide-in-from-bottom duration-300" style={{ fontFamily: '"Times New Roman", Times, serif', fontSize: '12pt' }}>
      <div className="bg-white border-b border-slate-300 shrink-0 flex justify-between items-center px-4 py-3 shadow-sm">
          <div className="flex items-center gap-2"><FileText className="w-5 h-5 text-teal-600" /><div><h2 className="font-black text-[13pt] uppercase tracking-tighter leading-none">SQC-VT FORM</h2><p className="text-[8pt] font-bold text-slate-400 uppercase tracking-widest mt-0.5">Gia công ngoài - Vật tư</p></div></div>
      ...
      <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-6 no-scrollbar">
        <div className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm space-y-4">
            <h3 className="text-teal-700 border-b border-teal-50 pb-2 mb-2 font-black text-xs uppercase tracking-widest flex items-center gap-2"><Box className="w-4 h-4"/> I. THÔNG TIN GIA CÔNG</h3>
        ...
        <div className="space-y-3">
            <h3 className="font-black text-slate-700 text-xs uppercase tracking-[0.2em] flex items-center gap-2 border-b border-slate-300 pb-2 px-1"><LayoutList className="w-4 h-4 text-teal-600"/> II. DANH MỤC KIỂM TRA CHẤT LƯỢNG VẬT TƯ</h3>
            <div className="space-y-4">
                {formData.items?.map((item, idx) => (
                    <div key={item.id} className="bg-white rounded-[2rem] p-5 border border-slate-200 shadow-sm">
                        ...
                        <textarea value={item.notes || ''} onChange={e => handleItemChange(idx, 'notes', e.target.value)} className="w-full mt-4 p-4 bg-slate-50 border border-slate-200 rounded-2xl text-sm outline-none h-16" placeholder="Ghi chú sai lệch..."/>
                    </div>
                ))}
            </div>
        </div>
      ...
      <div className="p-4 md:p-6 border-t border-slate-200 bg-white flex justify-end gap-3 shrink-0 shadow-sm z-20">
        ...
        <button onClick={handleSubmit} disabled={isSaving} className="px-16 py-4 bg-teal-700 text-white font-black uppercase text-xs tracking-[0.2em] rounded-2xl shadow-2xl hover:bg-teal-800 active:scale-95 transition-all flex items-center justify-center gap-3 disabled:opacity-50" type="button">
