    ...
    <div className="flex flex-col h-full overflow-hidden bg-slate-50">
      ...
      <div className="bg-white px-3 py-3 border-b border-slate-200 shadow-sm z-30 shrink-0 lg:px-6 lg:py-4">
        <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between lg:gap-4">
            ...
            <div className="flex flex-wrap items-center gap-2 w-full lg:flex-nowrap">
                ...
                <div className="relative" ref={moduleMenuRef}>
                    ...
                    >
                        <span className="text-[10px] font-black text-slate-900 uppercase truncate">{currentModuleName}</span>
                ...
                {/* 2. Status Filter Dropdown (Converted from Tabs as requested) */}
                <div className="relative" ref={filterMenuRef}>
                     <button 
                        onClick={() => setShowFilterMenu(!showFilterMenu)} 
                        className={`h-11 px-3 flex items-center justify-between gap-3 rounded-xl border shadow-sm transition-all active:scale-95 min-w-[120px] ${filter !== 'ALL' ? 'bg-blue-50 border-blue-200 text-blue-700' : 'bg-white text-slate-700 border-slate-200'}`}
                     >
                        <div className="flex items-center gap-2 overflow-hidden">
                            <ListFilter className="w-4 h-4 shrink-0 opacity-50" />
                            <span className="text-[10px] font-black uppercase truncate">{activeFilterLabel}</span>
                        </div>
                        <div className="flex items-center gap-1 shrink-0">
                            <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold ${filter !== 'ALL' ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-500'}`}>{activeFilterCount}</span>
                            ...
                            {filterOptions.map((opt) => (
                                ...
                                >
                                    <div className="flex items-center gap-3">
                                        {opt.icon || <div className="w-3 h-3" />}
                                        <span className="uppercase">{opt.label}</span>
                                    </div>
                                    <span className={`px-2 py-0.5 rounded-full text-[9px] font-black ${filter === opt.key ? 'bg-blue-600 text-white' : 'bg-slate-100 text-slate-400'}`}>
                                        {opt.count}
                                    </span>
                                </button>
                            ))}
                ...
                <div className="relative group flex-1 min-w-[200px]">
                  ...
                  <input 
                    ...
                    className="w-full pl-10 pr-20 h-11 bg-slate-50 border border-slate-200 rounded-xl text-xs font-bold text-slate-700 outline-none focus:bg-white focus:border-blue-500 focus:ring-4 focus:ring-blue-500/10 transition-all shadow-inner"
                  />
            ...
            {/* Bottom Row: Date Filter (Compact) */}
            <div className="flex items-center bg-slate-50 border border-slate-200 rounded-xl px-3 h-10 w-full lg:w-72 lg:h-11 lg:bg-white group">
                <Calendar className="w-3.5 h-3.5 text-slate-400 mr-2 group-hover:text-blue-500 transition-colors" />
                <input type="date" value={startDate} onChange={(e) => setStartDate(e.target.value)} className="bg-transparent text-[10px] font-bold text-slate-600 outline-none w-full cursor-pointer" />
                <span className="text-slate-300 mx-2 font-light">|</span>
                <input type="date" value={endDate} onChange={(e) => setEndDate(e.target.value)} className="bg-transparent text-[10px] font-bold text-slate-600 outline-none w-full text-right cursor-pointer" />
            </div>
        </div>
      </div>
