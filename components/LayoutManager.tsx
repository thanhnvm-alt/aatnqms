...
export const LayoutManager: React.FC<LayoutManagerProps> = ({ 
    ...
    const handleSelectPin = async (pin: LayoutPin) => {
        if (!pin.inspection_id || draggingPinId) return;
        setSelectedPin(pin);
    ...
    const handleMouseUp = async () => {
        ...
        if (draggingPinId) {
            ...
            if (pin) await saveLayoutPin(pin);
    ...
    const getPinColor = (status: string) => {
        switch (status) {
            case InspectionStatus.APPROVED: return 'from-emerald-400 to-emerald-600';
            case InspectionStatus.FLAGGED: return 'from-red-400 to-red-600';
            case InspectionStatus.PENDING: return 'from-amber-400 to-amber-600';
    ...
    return (
        <div className="absolute inset-0 z-40 bg-[#f1f5f9] flex flex-col animate-in fade-in duration-300 overflow-hidden">
            ...
            <div className="flex-1 relative overflow-hidden flex">
                    ...
                    <div className="flex-1 overflow-y-auto no-scrollbar p-2 space-y-1">
                        {filteredPins.map(pin => (
                            <div 
                                ...
                                className={`p-3 rounded-2xl border transition-all cursor-pointer group ${selectedPin?.id === pin.id ? 'bg-blue-50 border-blue-200 shadow-sm ring-1 ring-blue-100' : 'bg-white border-transparent hover:bg-slate-50 hover:border-slate-200'}`}
                            >
                                <div className="flex items-start gap-3">
                                    <div className={`w-8 h-8 rounded-xl flex items-center justify-center text-white shrink-0 shadow-sm bg-gradient-to-br ${getPinColor(pin.status)}`}>
                                        <MapPin className="w-4 h-4" />
                                    </div>
                                    <div className="flex-1 min-w-0">
                                        <h4 className={`text-[11px] font-black uppercase truncate pr-1 ${selectedPin?.id === pin.id ? 'text-blue-700' : 'text-slate-800'}`}>{pin.label || 'Chưa đặt tên'}</h4>
                                        <p className={`text-[8px] font-black uppercase mt-0.5 ${pin.status === InspectionStatus.APPROVED ? 'text-emerald-600' : pin.status === InspectionStatus.FLAGGED ? 'text-red-600' : 'text-amber-600'}`}>{pin.status}</p>
                                    </div>
                                </div>
                            </div>
                        ))}
                ...
                <div 
                    ref={containerRef}
                    className={`flex-1 flex items-center justify-center select-none overflow-hidden relative bg-[#cbd5e1] transition-all duration-300 ${isAddingMode ? 'cursor-crosshair' : scale > 1 ? 'cursor-grab active:cursor-grabbing' : 'cursor-default'}`}
                ...
                >
                    <div 
                        className="relative shadow-[0_40px_120px_rgba(0,0,0,0.2)] transition-transform duration-75 ease-out rounded-sm overflow-hidden bg-white"
                        style={{ 
                    ...
                    >
                        ...
                        {pins.map(pin => (
                            <div
                                key={pin.id}
                                onMouseDown={(e) => { e.stopPropagation(); setDraggingPinId(pin.id); }}
                                onClick={(e) => { e.stopPropagation(); handleSelectPin(pin); }}
                                className={`absolute group cursor-pointer transition-transform hover:scale-125 z-40 ${draggingPinId === pin.id ? 'scale-150 z-50' : ''}`}
                                style={{ left: `${pin.x}%`, top: `${pin.y}%`, transform: 'translate(-50%, -50%)' }}
                            >
                                <div className={`w-7 h-7 md:w-8 md:h-8 rounded-full flex items-center justify-center relative transition-all shadow-xl ${
                                    pin.status === InspectionStatus.APPROVED ? 'bg-emerald-50/20' : 
                                    pin.status === InspectionStatus.FLAGGED ? 'bg-red-50/20' : 'bg-blue-50/20'
                                }`}>
                                    <div className={`absolute inset-0 rounded-full border-2 animate-ping opacity-20 ${
                                        pin.status === InspectionStatus.APPROVED ? 'border-emerald-500' : 
                                        pin.status === InspectionStatus.FLAGGED ? 'border-red-500' : 'border-blue-500'
                                    }`}></div>
                                    <div className="w-3.5 h-3.5 md:w-4 md:h-4 bg-orange-500 rounded-full border-2 border-white shadow-md flex items-center justify-center">
                                        <div className="w-1 h-1 bg-white rounded-full"></div>
                                    </div>
                                    <div className="absolute top-full left-1/2 -translate-x-1/2 mt-2 hidden md:group-hover:block bg-slate-900 text-white text-[7px] font-black px-2 py-0.5 rounded whitespace-nowrap shadow-xl z-50 uppercase tracking-widest">
                        ...
                        <>
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <div className="flex items-center gap-2">
                                            <div className="w-2.5 h-2.5 bg-orange-500 rounded-full border-2 border-white shadow-sm animate-pulse"></div>
                                            <h3 className="font-black text-slate-900 text-xs uppercase tracking-tight">Hồ sơ #{quickDetail.id.split('-').pop()}</h3>
                                        </div>
                                        <div className="flex gap-1.5 mt-1">
                                            <span className={`px-2 py-0.5 rounded-full text-[8px] font-black uppercase border shadow-sm ${quickDetail.status === InspectionStatus.APPROVED ? 'bg-green-50 text-green-700 border-green-200' : 'bg-red-50 text-red-700 border-red-200'}`}>{quickDetail.status}</span>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="flex-1 overflow-y-auto no-scrollbar p-5 space-y-6 bg-slate-50/30">
                                ...
                                <div className="space-y-2">
                                    ...
                                    >
                                        <div className="w-12 h-12 rounded-2xl bg-slate-50 flex items-center justify-center border border-slate-100 text-slate-300 group-hover:text-blue-500 group-hover:bg-blue-50 transition-all">
            ...
            <input type="file" ref={quickCameraRef} accept="image/*" capture="environment" className="hidden" onChange={handleQuickAddImage} />
