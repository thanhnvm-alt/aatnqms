...
import { ArrowLeft, Box, Edit3, Trash2, MapPin, CheckCircle2, AlertOctagon, X, Loader2, Eraser, Building2 } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 overflow-hidden relative" style={{ fontFamily: '"Times New Roman", Times, serif' }}>
      <div className="bg-white border-b border-slate-200 p-3 sticky top-0 z-30 shadow-sm shrink-0 flex justify-between items-center">
          <div className="flex items-center gap-2"><button onClick={onBack} className="p-2 hover:bg-slate-100 rounded-xl transition-colors border border-slate-200"><ArrowLeft className="w-4 h-4 text-slate-600" /></button><span className="text-sm font-bold text-slate-900 uppercase">SITE REPORT</span></div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50 pb-24">
        <div className="bg-white p-4 rounded-xl border border-amber-200 shadow-sm">
            <h1 className="text-lg font-bold text-amber-800 uppercase mb-2 flex items-center gap-2"><MapPin className="w-5 h-5 text-amber-600"/> {inspection.ten_ct}</h1>
            <div className="grid grid-cols-2 gap-2 text-[10px] font-bold text-slate-600">
                <p>Mã CT: {inspection.ma_ct}</p>
                <p>Địa điểm: {inspection.location || 'N/A'}</p>
            </div>
        </div>
        ...
        <div className="bg-white p-4 rounded-xl border border-slate-200 mt-2">
            <h3 className="text-[10px] font-bold uppercase text-slate-500 border-l-4 border-amber-500 pl-2 mb-2">Nghiệm thu lắp đặt</h3>
      ...
      {!isApproved && isManager && <div className="fixed bottom-0 left-0 right-0 p-4 bg-white border-t border-slate-200 flex justify-end z-40"><button onClick={() => setShowModal(true)} className="bg-amber-700 text-white px-6 py-3 rounded-xl font-bold uppercase text-[10px] shadow-lg">DUYỆT SITE</button></div>}
      {showModal && <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4"><div className="bg-white p-6 rounded-2xl w-full max-w-sm space-y-4"><h3 className="font-bold uppercase text-sm">Xác nhận Nghiệm Thu</h3><SignaturePad label="Chữ ký Manager" value={managerSig} onChange={setManagerSig}/><div className="flex gap-2"><button onClick={() => setShowModal(false)} className="flex-1 py-2 border rounded-lg text-xs font-bold uppercase">Hủy</button><button onClick={handleApprove} disabled={!managerSig || isProcessing} className="flex-1 py-2 bg-amber-700 text-white rounded-lg text-xs font-bold uppercase">{isProcessing ? <Loader2 className="w-4 h-4 animate-spin mx-auto"/> : 'XÁC NHẬN'}</button></div></div></div>}
    </div>
  );
};
