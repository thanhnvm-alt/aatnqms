...
import { ArrowLeft, Box, Edit3, Trash2, MapPin, CheckCircle2, AlertOctagon, X, Loader2, Eraser, Building2, Image as ImageIcon, Maximize2 } from 'lucide-react';
...
export const InspectionDetailSITE: React.FC<InspectionDetailProps> = ({ inspection, user, onBack, onEdit, onDelete, onApprove }) => {
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 overflow-hidden relative" style={{ fontFamily: '"Times New Roman", Times, serif' }}>
      <div className="bg-white border-b border-slate-200 p-3 sticky top-0 z-30 shadow-sm shrink-0 flex justify-between items-center">
          <div className="flex items-center gap-2"><button onClick={onBack} className="p-2 hover:bg-slate-100 rounded-xl transition-colors border border-slate-200 shadow-sm" type="button"><ArrowLeft className="w-4 h-4 text-slate-600" /></button><span className="text-sm font-bold text-slate-900 uppercase tracking-tight">SITE QC REPORT REVIEW</span></div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50 pb-24 no-scrollbar">
        <div className="bg-white p-4 rounded-xl border border-amber-200 shadow-sm relative overflow-hidden">
            <h1 className="text-lg font-bold text-amber-900 uppercase mb-2 flex items-center gap-2"><MapPin className="w-5 h-5 text-amber-600"/> {inspection.ten_ct}</h1>
            <div className="flex flex-col gap-1 text-[10px] font-bold text-slate-500 uppercase tracking-widest">
                <p>Mã CT: {inspection.ma_ct}</p>
                <p>Ngày nghiệm thu: {inspection.date}</p>
                <p className="flex items-center gap-1"><MapPin className="w-3 h-3"/> Địa điểm: {inspection.location || 'Hồ Chí Minh'}</p>
            </div>
            <div className="absolute right-4 bottom-2 text-4xl font-black text-amber-100 opacity-30 uppercase select-none">Site QC</div>
        </div>

        {/* Global Evidence */}
        {inspection.images && inspection.images.length > 0 && (
            <div className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm">
                <h3 className="text-[9px] font-black text-slate-400 uppercase tracking-widest mb-2 flex items-center gap-1.5"><ImageIcon className="w-3 h-3"/> Nhật ký hình ảnh hiện trường</h3>
                <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar">
                    {inspection.images.map((img, idx) => (
                        <div key={idx} onClick={() => setLightboxState({ images: inspection.images!, index: idx })} className="w-20 h-20 rounded-xl overflow-hidden border border-slate-100 shrink-0 cursor-zoom-in">
        ...
        <div className="space-y-2">
            <h3 className="text-[10px] font-black text-slate-400 uppercase tracking-widest px-1">Danh mục kiểm tra lắp đặt</h3>
            {inspection.items.map((item, i) => (
                <div key={i} className={`bg-white p-4 rounded-xl border shadow-sm ${item.status === CheckStatus.FAIL ? 'border-red-200 bg-red-50/10' : 'border-slate-200'}`}>
                            ...
                            {item.images && item.images.length > 0 && (
                                <div className="flex gap-2 mt-3 overflow-x-auto no-scrollbar py-1">
                                    {item.images.map((img, imgIdx) => (
                                        <div key={imgIdx} onClick={() => setLightboxState({ images: item.images!, index: imgIdx })} className="w-14 h-14 rounded-lg overflow-hidden border border-slate-100 shrink-0 cursor-zoom-in shadow-sm">
                                            <img src={img} className="w-full h-full object-cover" alt=""/>
                                        </div>
                                    ))}
                                </div>
        ...
        <div className="bg-white p-4 rounded-2xl border border-slate-200 mt-2 shadow-sm">
            <h3 className="text-[10px] font-bold uppercase text-slate-500 border-l-4 border-amber-500 pl-2 mb-4">Ký tên & Xác nhận nghiệm thu</h3>
            <div className="grid grid-cols-2 gap-4">
                <div className="text-center space-y-2">
                    <div className="bg-slate-50 h-24 rounded-2xl flex items-center justify-center border border-slate-100 shadow-inner overflow-hidden">
                    ...
                    <span className="text-[10px] font-black uppercase text-slate-800 tracking-tight">{inspection.inspectorName}</span>
                    <p className="text-[8px] font-bold text-slate-400 uppercase tracking-[0.2em]">QC Hiện Trường</p>
                </div>
                <div className="text-center space-y-2">
                    <div className="bg-slate-50 h-24 rounded-2xl flex items-center justify-center border border-slate-100 shadow-inner overflow-hidden">
                    ...
                    <span className="text-[10px] font-black uppercase text-slate-800 tracking-tight">Manager Approval</span>
                    <p className="text-[8px] font-bold text-slate-400 uppercase tracking-[0.2em]">Cấp Quản Lý</p>
                </div>
            </div>
        </div>
      </div>
      ...
      {!isApproved && isManager && (
        <div className="fixed bottom-0 left-0 right-0 p-4 bg-white/90 backdrop-blur-md border-t border-slate-200 flex justify-end z-40 shadow-lg">
            <button onClick={() => setShowModal(true)} className="bg-amber-600 text-white px-8 py-3 rounded-xl font-black uppercase text-[10px] shadow-xl shadow-amber-200 active:scale-95 transition-all">DUYỆT NGHIỆM THU SITE</button>
        </div>
      )}

      {showModal && (
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[200] flex items-center justify-center p-4">
            <div className="bg-white p-6 rounded-[2.5rem] w-full max-w-sm space-y-5 animate-in zoom-in duration-200 shadow-2xl border border-white">
                <div className="flex justify-between items-center px-1">
                    <h3 className="font-black uppercase text-sm text-slate-800 tracking-tight">Manager Xác nhận</h3>
                ...
                <div className="flex gap-3 pt-2">
                    <button onClick={() => setShowModal(false)} className="flex-1 py-3.5 border border-slate-200 rounded-2xl text-[10px] font-bold uppercase text-slate-500 hover:bg-slate-50">Đóng</button>
                    <button onClick={handleApprove} disabled={!managerSig || isProcessing} className="flex-[2] py-3.5 bg-amber-600 text-white rounded-2xl text-[10px] font-black uppercase shadow-xl shadow-amber-100 disabled:opacity-50">
                        {isProcessing ? <Loader2 className="w-4 h-4 animate-spin mx-auto"/> : 'XÁC NHẬN DUYỆT'}
                    </button>
                </div>
            </div>
        </div>
