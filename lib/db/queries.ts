...
export const getPlans = async (params: { search?: string, page?: number, limit?: number }) => { 
    ...
    return { items: dataRes.rows as unknown as PlanItem[], total: Number(countRes.rows[0]?.total || 0) }; 
};
...
export const getProjects = async (): Promise<Project[]> => {
    ...
    try {
        ...
        metaRes.rows.forEach(row => { try { if (row.data) metaMap[row.ma_ct as string] = JSON.parse(row.data as string); } catch(e) {} });
        return derivedRes.rows.map(row => {
            ...
            return { id: `proj_${ma_ct}`, code: ma_ct, ma_ct: ma_ct, name: ten_ct, ten_ct: ten_ct, status: 'Planning', pm: 'Chưa phân công', pc: '---', qa: '---', location: '', startDate: new Date().toISOString().split('T')[0], endDate: '', thumbnail: 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?q=80&w=400', progress: 0, description: '', images: [] } as Project;
        });
...
export const updateProject = async (project: Project) => {
  if (!isTursoConfigured) return;
  const now = Math.floor(Date.now() / 1000);
  await turso.execute({ sql: `INSERT INTO projects (ma_ct, data, updated_at) VALUES (?, ?, ?) ON CONFLICT(ma_ct) DO UPDATE SET data = excluded.data, updated_at = excluded.updated_at`, args: [project.ma_ct, JSON.stringify(project), now] });
};
