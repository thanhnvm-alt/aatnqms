...
export const initDatabase = async () => {
  ...
  try {
    // Retry initialization connection to ensure DB is awake
...
export const getRoles = async (): Promise<Role[]> => { if (!isTursoConfigured) return []; const res: any = await withRetry(() => turso.execute("SELECT data FROM roles")); return res.rows.map((r: any) => JSON.parse(r.data as string)); };
export const saveRole = async (role: Role) => { if (!isTursoConfigured) return; const now = Math.floor(Date.now() / 1000); await withRetry(() => turso.execute({ sql: `INSERT INTO roles (id, data, created_at, updated_at) VALUES (?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET data = excluded.data, updated_at = excluded.updated_at`, args: [role.id, JSON.stringify(role), now, now] })); };
export const deleteRole = async (id: string) => { if (!isTursoConfigured) return; await withRetry(() => turso.execute({ sql: "DELETE FROM roles WHERE id = ?", args: [id] })); };
...
export const getPlans = async (params: { search?: string, page?: number, limit?: number }) => {
  ...
  let args: any[] = [];
  if (search) {
    sql += " WHERE ma_ct LIKE ? OR ma_nha_may LIKE ? OR headcode LIKE ? OR ten_hang_muc LIKE ?";
    const term = `%${search}%`;
  ...
  return await withRetry(async () => {
    ...
    // Explicitly handle as any to avoid type errors
    const dataRes = results[0];
    const countRes = results[1];

    return {
      items: dataRes.rows as unknown as PlanItem[],
      total: Number(countRes.rows[0]?.total || 0)
    };
  });
};

export const importPlansBatch = async (plans: PlanItem[]) => { if (!isTursoConfigured) return; await withRetry(async () => { for (const p of plans) { await turso.execute({ sql: `INSERT INTO plans (stt, ma_nha_may, headcode, ma_ct, ten_ct, ten_hang_muc, dvt, so_luong_ipo, plannedDate, assignee, status, pthsp, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, args: [p.stt, p.ma_nha_may, p.headcode, p.ma_ct, p.ten_ct, p.ten_hang_muc, p.dvt, p.so_luong_ipo, p.plannedDate, p.assignee, p.status, p.pthsp, Math.floor(Date.now()/1000)] }); } }); };
export const deleteInspection = async (id: string) => { if (!isTursoConfigured) return; await withRetry(() => turso.execute({ sql: "DELETE FROM inspections WHERE id = ?", args: [id] })); };
export const getUsers = async (): Promise<User[]> => { if (!isTursoConfigured) return []; const res: any = await withRetry(() => turso.execute("SELECT data FROM users")); return res.rows.map((r: any) => JSON.parse(r.data as string)); };
export const saveUser = async (user: User) => { if (!isTursoConfigured) return; const now = Math.floor(Date.now() / 1000); await withRetry(() => turso.execute({ sql: `INSERT INTO users (id, data, username, role, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET data = excluded.data, username = excluded.username, role = excluded.role, updated_at = excluded.updated_at`, args: [user.id, JSON.stringify(user), user.username, user.role, now, now] })); };
export const deleteUser = async (id: string) => { if (!isTursoConfigured) return; await withRetry(() => turso.execute({ sql: "DELETE FROM users WHERE id = ?", args: [id] })); };
export const getWorkshops = async (): Promise<Workshop[]> => { if (!isTursoConfigured) return []; const res: any = await withRetry(() => turso.execute("SELECT data FROM workshops")); return res.rows.map((r: any) => JSON.parse(r.data as string)); };
export const saveWorkshop = async (ws: Workshop) => { if (!isTursoConfigured) return; const now = Math.floor(Date.now() / 1000); await withRetry(() => turso.execute({ sql: `INSERT INTO workshops (id, data, code, name, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET data = excluded.data, code = excluded.code, name = excluded.name, updated_at = excluded.updated_at`, args: [ws.id, JSON.stringify(ws), ws.code, ws.name, now, now] })); };
export const deleteWorkshop = async (id: string) => { if (!isTursoConfigured) return; await withRetry(() => turso.execute({ sql: "DELETE FROM workshops WHERE id = ?", args: [id] })); };
export const getTemplates = async (): Promise<Record<string, CheckItem[]>> => { if (!isTursoConfigured) return {}; const res: any = await withRetry(() => turso.execute("SELECT moduleId, data FROM templates")); const result: Record<string, CheckItem[]> = {}; res.rows.forEach((r: any) => { result[r.moduleId as string] = JSON.parse(r.data as string); }); return result; };
export const saveTemplate = async (moduleId: string, items: CheckItem[]) => { if (!isTursoConfigured) return; const now = Math.floor(Date.now() / 1000); await withRetry(() => turso.execute({ sql: `INSERT INTO templates (moduleId, data, updated_at) VALUES (?, ?, ?) ON CONFLICT(moduleId) DO UPDATE SET data = excluded.data, updated_at = excluded.updated_at`, args: [moduleId, JSON.stringify(items), now] })); };
...
export const getNcrs = async (params: { inspection_id?: string, status?: string, page?: number, limit?: number }): Promise<NCR[]> => { 
    ...
    const res: any = await withRetry(() => turso.execute({ sql, args })); 
    return res.rows.map((row: any) => ({ id: row.id as string, inspection_id: row.inspection_id as string, createdDate: new Date((row.created_at as number) * 1000).toISOString().split('T')[0], issueDescription: row.description as string, rootCause: (row.root_cause as string) || '', solution: (row.corrective_action as string) || '', responsiblePerson: (row.responsible_person as string) || '', deadline: (row.deadline as string) || '', status: row.status as string, severity: row.severity as any, imagesBefore: [], imagesAfter: [], comments: [], itemId: row.item_id as string })); 
};

export const getNcrById = async (id: string): Promise<NCR | null> => {
    if (!isTursoConfigured) return null;
    return await withRetry(async () => {
        const res: any = await turso.execute({ sql: "SELECT * FROM ncrs WHERE id = ?", args: [id] });
        if (res.rows.length === 0) return null;
...
export const getDefectLibrary = async (): Promise<DefectLibraryItem[]> => { if (!isTursoConfigured) return []; const res: any = await withRetry(() => turso.execute("SELECT * FROM defect_library ORDER BY COALESCE(defect_code, id) ASC")); return res.rows.map((row: any) => ({ id: row.id as string, code: (row.defect_code as string) || (row.id as string), name: (row.name as string) || '', stage: (row.stage as string) || 'Chung', category: (row.category as string) || 'Khác', description: (row.description as string) || '', severity: (row.severity as any) || 'MINOR', suggestedAction: (row.suggested_action as string) || '', correctImage: (row.correct_image as string) || '', incorrectImage: (row.incorrect_image as string) || '', createdBy: (row.created_by as string) || 'System', createdAt: (row.created_at as number) || Math.floor(Date.now() / 1000) })); };
export const saveDefectLibraryItem = async (item: DefectLibraryItem) => { if (!isTursoConfigured) return; const now = Math.floor(Date.now() / 1000); await withRetry(() => turso.execute({ sql: `INSERT INTO defect_library (id, defect_code, name, stage, category, description, severity, suggested_action, correct_image, incorrect_image, created_by, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET defect_code = excluded.defect_code, name = excluded.name, stage = excluded.stage, category = excluded.category, description = excluded.description, severity = excluded.severity, suggested_action = excluded.suggested_action, correct_image = excluded.correct_image, incorrect_image = excluded.incorrect_image`, args: [item.id, item.code, item.name || '', item.stage, item.category, item.description, item.severity, item.suggestedAction || '', item.correctImage || '', item.incorrectImage || '', item.createdBy || 'System', item.createdAt || now] })); };
export const deleteDefectLibraryItem = async (id: string) => { if (!isTursoConfigured) return; await withRetry(() => turso.execute({ sql: "DELETE FROM defect_library WHERE id = ?", args: [id] })); };
...
export const getDefects = async (params: { search?: string, status?: string }): Promise<Defect[]> => {
    ...
    const res = await withRetry(() => turso.execute({ sql, args })) as any;
    return res.rows.map((row: any) => ({ id: row.id as string, inspectionId: row.inspectionId as string, itemId: row.itemId as string, defectCode: row.defectCode as string, category: 'N/A', description: row.description as string, status: row.status as string, severity: row.severity as string, inspectorName: row.inspectorName as string, date: row.date as string, ma_ct: (row.ma_ct as string) || 'Unknown', ten_ct: (row.ten_ct as string) || 'Unknown', images: JSON.parse((row.images_before_json as string) || '[]'), rootCause: (row.rootCause as string) || '', solution: (row.solution as string) || '', responsiblePerson: (row.responsiblePerson as string) || '', deadline: (row.deadline as string) || '' }));
};
