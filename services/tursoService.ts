...
export const getProjects = async (): Promise<Project[]> => {
    ...
    try {
        ...
        return derivedRes.rows.map(row => {
            ...
            return {
                id: `proj_${ma_ct}`, code: ma_ct, ma_ct, name: ten_ct, ten_ct, status: 'Planning', pm: 'Chưa phân công', pc: '---', qa: '---', location: '', startDate: new Date().toISOString().split('T')[0], endDate: '', thumbnail: 'https://images.unsplash.com/photo-1486406146926-c627a92ad1ab?q=80&w=400', progress: 0, description: '', images: []
            } as Project;
        });
    } catch (err) { return []; }
};
...
export const getNcrs = async (params: { inspection_id?: string, status?: string, page?: number, limit?: number }): Promise<NCR[]> => { if (!isTursoConfigured) return []; const { inspection_id, status, page = 1, limit = 50 } = params; const offset = (page - 1) * limit; let sql = `SELECT id, inspection_id, item_id, defect_code, severity, status, description, root_cause, corrective_action, responsible_person, deadline, created_by, created_at, updated_at FROM ncrs WHERE deleted_at IS NULL`; let args: any[] = []; if (inspection_id) { sql += ` AND inspection_id = ?`; args.push(inspection_id); } if (status && status !== 'ALL') { sql += ` AND status = ?`; args.push(status); } sql += ` ORDER BY created_at DESC LIMIT ? OFFSET ?`; args.push(limit, offset); const res = await turso.execute({ sql, args }); return res.rows.map(row => ({ id: row.id as string, inspection_id: row.inspection_id as string, createdDate: new Date((row.created_at as number) * 1000).toISOString().split('T')[0], issueDescription: row.description as string, rootCause: (row.root_cause as string) || '', solution: (row.corrective_action as string) || '', responsiblePerson: (row.responsible_person as string) || '', deadline: (row.deadline as string) || '', status: row.status as string, severity: row.severity as any, imagesBefore: [], imagesAfter: [], comments: [], itemId: row.item_id as string })); };
export const getNcrById = async (id: string): Promise<NCR | null> => { if (!isTursoConfigured) return null; const res = await turso.execute({ sql: "SELECT * FROM ncrs WHERE id = ?", args: [id] }); if (res.rows.length === 0) return null; const row = res.rows[0]; return { id: row.id as string, inspection_id: row.inspection_id as string, createdDate: new Date((row.created_at as number) * 1000).toISOString().split('T')[0], issueDescription: row.description as string, rootCause: (row.root_cause as string) || '', solution: (row.corrective_action as string) || '', responsiblePerson: (row.responsible_person as string) || '', deadline: (row.deadline as string) || '', status: row.status as string, severity: row.severity as any, imagesBefore: JSON.parse((row.images_before_json as string) || '[]'), imagesAfter: JSON.parse((row.images_after_json as string) || '[]'), comments: JSON.parse((row.comments_json as string) || '[]'), itemId: row.item_id as string }; };
...
export const getDefects = async (params: { search?: string, status?: string }): Promise<Defect[]> => { if (!isTursoConfigured) return []; const { search = '', status = '' } = params; let sql = `SELECT n.id, n.inspection_id as inspectionId, n.item_id as itemId, n.defect_code as defectCode, n.severity, n.status, n.description, n.root_cause as rootCause, n.corrective_action as solution, n.responsible_person as responsiblePerson, n.deadline, n.created_by as inspectorName, strftime('%Y-%m-%d', datetime(n.created_at, 'unixepoch')) as date, n.images_before_json, i.ma_ct, i.ten_ct FROM ncrs n LEFT JOIN inspections i ON n.inspection_id = i.id WHERE n.deleted_at IS NULL`; let args: any[] = []; if (search) { sql += " AND (n.description LIKE ? OR n.defect_code LIKE ? OR i.ma_ct LIKE ?)"; const term = `%${search}%`; args.push(term, term, term); } if (status && status !== 'ALL') { sql += " AND n.status = ?"; args.push(status); } sql += " ORDER BY n.created_at DESC"; const res = await turso.execute({ sql, args }); return res.rows.map(row => ({ id: row.id as string, inspectionId: row.inspectionId as string, itemId: row.itemId as string, defectCode: row.defectCode as string, category: 'N/A', description: row.description as string, status: row.status as string, severity: row.severity as string, inspectorName: row.inspectorName as string, date: row.date as string, ma_ct: (row.ma_ct as string) || 'Unknown', ten_ct: (row.ten_ct as string) || 'Unknown', images: JSON.parse((row.images_before_json as string) || '[]'), rootCause: (row.rootCause as string) || '', solution: (row.solution as string) || '', responsiblePerson: (row.responsiblePerson as string) || '', deadline: (row.deadline as string) || '' })); };
export const importUsers = async (users: User[]) => { if (!isTursoConfigured) return; for (const user of users) await saveUser(user); };
export const saveNcrMapped = async (inspection_id: string, ncr: NCR, createdBy: string) => { if (!isTursoConfigured) return ''; const now = Math.floor(Date.now() / 1000); const id = ncr.id || `NCR-${Date.now()}`; await turso.execute({ sql: `INSERT INTO ncrs (id, inspection_id, item_id, defect_code, severity, status, description, root_cause, corrective_action, responsible_person, deadline, images_before_json, images_after_json, comments_json, created_by, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(id) DO UPDATE SET severity = excluded.severity, status = excluded.status, description = excluded.description, root_cause = excluded.root_cause, corrective_action = excluded.corrective_action, responsible_person = excluded.responsible_person, deadline = excluded.deadline, images_before_json = excluded.images_before_json, images_after_json = excluded.images_after_json, comments_json = excluded.comments_json, updated_at = excluded.updated_at`, args: [id, inspection_id, ncr.itemId || '', ncr.defect_code || '', ncr.severity || 'MINOR', ncr.status || 'OPEN', ncr.issueDescription, ncr.rootCause || '', ncr.solution || '', ncr.responsiblePerson || '', ncr.deadline || '', JSON.stringify(ncr.imagesBefore || []), JSON.stringify(ncr.imagesAfter || []), JSON.stringify(ncr.comments || []), createdBy, now, now] }); return id; };
