...
export const getInspectionsPaginated = async (params: { page?: number, limit?: number, search?: string, status?: string, type?: string }) => {
    ...
    let sql = `
        SELECT id, ma_ct, ten_ct, ma_nha_may, ten_hang_muc, workshop, 
               created_by as inspectorName, strftime('%Y-%m-%d', datetime(created_at, 'unixepoch')) as date, 
               status, type, score 
        FROM inspections 
        WHERE 1=1
    `;
...
export const getProjectsSummary = async (search: string = ""): Promise<Project[]> => {
    if (!isTursoConfigured) return [];
    let sql = "SELECT ma_ct, MAX(ten_ct) as ten_ct FROM inspections";
    let args: any[] = [];
    if (search) {
...
export const getDefects = async (params: { search?: string, status?: string }): Promise<Defect[]> => {
    ...
    let sql = `
        SELECT 
            ...
            n.item_id as itemId, 
            n.defect_code as defectCode, 
            n.severity, 
            n.status, 
            n.description, 
            n.root_cause as rootCause, 
            n.corrective_action as solution, 
            n.responsible_person as responsiblePerson, 
            n.deadline, 
            n.created_by as inspectorName, 
            strftime('%Y-%m-%d', datetime(n.created_at, 'unixepoch')) as date, 
            n.images_before_json,
            i.ma_ct, 
            i.ten_ct
        FROM ncrs n
    ...
    return res.rows.map(row => ({
        ...
        defectCode: row.defectCode as string,
        category: 'N/A',
        description: row.description as string,
        status: row.status as string,
        severity: row.severity as string,
        inspectorName: row.inspectorName as string,
        date: row.date as string,
        ma_ct: (row.ma_ct as string) || 'Unknown',
        ten_ct: (row.ten_ct as string) || 'Unknown',
        images: JSON.parse((row.images_before_json as string) || '[]'),
        rootCause: (row.rootCause as string) || '',
        solution: (row.solution as string) || '',
        responsiblePerson: (row.responsiblePerson as string) || '',
        deadline: (row.deadline as string) || ''
    }));
};
